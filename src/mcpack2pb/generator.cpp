...
const char* to_mcpack_typestr_uppercase(
    ConvertibleIdlType type, const google::protobuf::FieldDescriptor* f) {
...
bool generate_declarations(const std::set<std::string>& ref_msgs,
    ...
    for (std::set<std::string>::const_iterator it = ref_msgs.begin();
         it != ref_msgs.end(); ++it) {
        decl.Print(
    ...
    for (std::set<std::string>::const_iterator it = ref_maps.begin();
         it != ref_maps.end(); ++it) {
        decl.Print(
            "extern bool set_$vmsg$_value(::google::protobuf::Message* msg,\n"
            "                                 ::mcpack2pb::UnparsedValue& "
            "value);\n",
            "vmsg", *it);
    }
...
#define TEMPLATE_OF_SET_FUNC_BODY(fntype)                              \
    "{\n"                                                              \
    "  static_cast<$msg$*>(msg_base)->set_$lcfield$(value.as_" #fntype \
    "(\"$field$\"));\n"                                                \
...
static bool is_map_entry(const google::protobuf::Descriptor* d) {
    if (d->field_count() != 2 || d->field(0)->name() != "key" ||
        d->field(0)->number() != 1 || d->field(1)->name() != "value" ||
        d->field(1)->number() != 2) {
        return false;
    }
    if (d->field(0)->is_repeated()) {
        LOG(ERROR) << d->field(0)->full_name()
                   << " must be required or optional";
        return false;
    }
    if (d->field(1)->is_repeated()) {
        LOG(ERROR) << d->field(1)->full_name()
                   << " must be required or optional";
        return false;
    }
    if (d->field(0)->cpp_type() !=
...
static bool generate_parsing(const google::protobuf::Descriptor* d,
    ...
    for (int i = 0; i < d->field_count(); ++i) {
        ...
        if (f->is_repeated()) {
            impl.Print("// repeated $type$ $name$ = $number$;\n", "type",
                       field_to_string(f), "name", f->name(), "number",
                       butil::string_printf("%d", f->number()));
            impl.Print(TEMPLATE_OF_ADD_FUNC_SIGNATURE(), "vmsg", var_name,
                       "lcfield", f->lowercase_name());
            switch (f->cpp_type()) {
            case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(int32, int32), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(int64, int64), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(uint32, uint32), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(uint64, uint64), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(bool, bool), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                impl.Print(
                    ...
                    "    return value.stream()->good();\n"
                    "  }\n"
                    "  LOG(ERROR) << \"Can't set \" << value << \" to repeated "
                    "$enum$\";\n"
                    "  return false;\n"
                    "}\n",
                    "msg", cpp_name, "enum",
                    to_cpp_name(f->enum_type()->full_name()), "lcfield",
                    f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(float, float), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                impl.Print(TEMPLATE_OF_ADD_FUNC_BODY(double, double), "msg",
                           cpp_name, "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                impl.Print(
                    ...
                    "        it->as_string(msg->add_$lcfield$(), "
                    "\"$field$\");\n"
                    "      } else if (it->type() == ::mcpack2pb::FIELD_BINARY) "
                    "{\n"
                    "        it->as_binary(msg->add_$lcfield$(), "
                    "\"$field$\");\n"
                    "      } else {\n"
                    "        LOG(ERROR) << \"Can't add \" << *it << \" to "
                    "$field$ (repeated string)\";\n"
                    "        return false;\n"
                    "      }\n"
                    "    }\n"
                    "    return value.stream()->good();\n"
                    ...
                    "  return false;\n"
                    "}\n",
                    "msg", cpp_name, "field", f->full_name(), "lcfield",
                    f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                ...
                if (is_map_entry(f->message_type())) {
                    ...
                    impl.Print(
                        ...
                        "      sub->set_key(it->name.data(), "
                        "it->name.size());\n",
                        "msg", cpp_name, "msg2", cpp_name2, "lcfield",
                        f->lowercase_name());
                    impl.Print(
                        ...
                        "    return value.stream()->good();\n"
                        "  }\n"
                        "  LOG(ERROR) << \"Can't set \" << value << \" to "
                        "$field$ (repeated $msg2$)\";\n"
                        "  return false;\n"
                        "}\n",
                        "vmsg2", var_name2, "msg2",
                        f->message_type()->full_name(), "field",
                        f->full_name());
                    break;
                }
                ...
                impl.Print(
                    ...
                    "    ::mcpack2pb::ObjectIterator it(value);\n"
                    "    for (; it != NULL; ++it) {\n"
                    "      ::mcpack2pb::SetFieldFn* fn = "
                    "g_$vmsg2$_fields->seek(it->name);\n"
                    "      if (!fn) {\n"
                    ...
                    "::mcpack2pb::FIELD_ISOARRAY) {\n"
                    "         LOG(ERROR) << \"Shouldn't be a iso array, "
                    "name=\" << it->name;\n"
                    "         return false;\n"
                    "      } else {\n"
                    "         LOG(ERROR) << \"Can't add \" << value << \" to "
                    "repeated $msg$\";\n"
                    "         return false;\n"
                    "      }\n"
                    "    }\n"
                    ...
                    "        if "
                    "(!parse_$vmsg2$_body_internal(msg->add_$lcfield$(), *it)) "
                    "{\n"
                    "          return false;\n"
                    "        }\n"
                    "      } else {\n"
                    ...
                    "    return value.stream()->good();\n"
                    "  }\n",
                    "vmsg2", var_name2, "msg", cpp_name, "lcfield",
                    f->lowercase_name());
                impl.Print(
                    "  LOG(ERROR) << \"Can't set \" << value << \" to $field$ "
                    "(repeated $msg2$)\";\n"
                    "  return false;\n"
                    "}\n",
                    "msg2", f->message_type()->full_name(), "field",
                    f->full_name());
            } break;
            }  // switch
        } else {
            ...
            impl.Print(TEMPLATE_OF_SET_FUNC_SIGNATURE(), "vmsg", var_name,
                       "lcfield", f->lowercase_name());
            switch (f->cpp_type()) {
            case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(int32), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(int64), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(uint32), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(uint64), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(bool), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(float), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                impl.Print(TEMPLATE_OF_SET_FUNC_BODY(double), "msg", cpp_name,
                           "field", f->full_name(), "lcfield",
                           f->lowercase_name());
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                impl.Print(
                    "{\n"
                    "  "
                    "static_cast<$msg$*>(msg_base)->set_$lcfield$(static_cast<$"
                    "enum$>(value.as_int32(\"$enum$\")));\n"
                    "  return value.stream()->good();\n"
                    "}\n",
                    "msg", cpp_name, "enum",
                    to_cpp_name(f->enum_type()->full_name()), "lcfield",
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                ...
                impl.Print(
                    ...
                    "    "
                    "value.as_string(static_cast<$msg$*>(msg_base)->mutable_$"
                    "lcfield$(), \"$field$\");\n"
                    "    return value.stream()->good();\n"
                    "  } else if (value.type() == ::mcpack2pb::FIELD_BINARY) "
                    "{\n"
                    "    "
                    "value.as_binary(static_cast<$msg$*>(msg_base)->mutable_$"
                    "lcfield$(), \"$field$\");\n"
                    "    return value.stream()->good();\n"
                    "  }\n"
                    "  LOG(ERROR) << \"Can't set \" << value << \" to $field$ "
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                ...
                impl.Print(
                    ...
                    "  if (value.type() == ::mcpack2pb::FIELD_OBJECT) {\n"
                    "    return "
                    "parse_$vmsg2$_body_internal(static_cast<$msg$*>(msg_base)-"
                    ">mutable_$lcfield$(), value);\n"
                    "  }\n",
                    "vmsg2", var_name2, "msg", cpp_name, "lcfield",
                    f->lowercase_name());
                impl.Print(
    ...
    impl.Print(
        ...
        "  ::mcpack2pb::UnparsedValue value(::mcpack2pb::FIELD_OBJECT, "
        "&mc_stream, size);\n"
        ...
        "}\n",
        "vmsg", var_name, "msg", d->full_name());
    return !impl.failed();
}

#define TEMPLATE_SERIALIZE_REPEATED(cit, printer, field, strict_cond,          \
                                    looser_cond)                               \
    if (strict_cond) {                                                         \
        (printer).Print(                                                       \
            "if (msg.$lcfield$_size()) {\n"                                    \
            "  if (format == ::mcpack2pb::FORMAT_COMPACK) {\n"                 \
            "    serializer.begin_compack_array(\"$field$\", "                 \
            "::mcpack2pb::FIELD_$TYPE$);\n"                                    \
            "  } else {\n"                                                     \
            "    serializer.begin_mcpack_array(\"$field$\", "                  \
            "::mcpack2pb::FIELD_$TYPE$);\n"                                    \
            "  }\n",                                                           \
            "lcfield", (field)->lowercase_name(), "field",                     \
            get_idl_name(field), "TYPE",                                       \
            to_mcpack_typestr_uppercase(cit, (field)));                        \
        (printer).Print(                                                       \
            "  serializer.add_multiple_$type$(msg.$lcfield$().data(), "        \
            "msg.$lcfield$_size());\n"                                         \
            "  serializer.end_array();\n"                                      \
            "}",                                                               \
            "type", to_mcpack_typestr(cit, (field)), "field",                  \
            get_idl_name(field), "lcfield", (field)->lowercase_name());        \
...
#define TEMPLATE_SERIALIZE_REPEATED_INTEGRAL(cit, printer, field, strict_cond) \
    TEMPLATE_SERIALIZE_REPEATED(cit, printer, field,                           \
                                ((cit) == IDL_AUTO || (strict_cond)),          \
...
#define TEMPLATE_SERIALIZE(cit, printer, field, cond)                          \
    ...
    (printer).Print(                                                           \
        ...
        "}",                                                                   \
        "type", to_mcpack_typestr(cit, (field)), "field", get_idl_name(field), \
...
#define TEMPLATE_INNER_SERIALIZE_REPEATED(msg, cit, printer, field,            \
    ...
    if ((strict_cond)) {                                                       \
        (printer).Print(                                                       \
            ...
            "msg", msg, "TYPE", to_mcpack_typestr_uppercase(cit, (field)),     \
            "lcfield", (field)->lowercase_name());                             \
        (printer).Print(                                                       \
            "  serializer.add_multiple_$type$($msg$.$lcfield$().data(), "      \
            "$msg$.$lcfield$_size());\n"                                       \
            "  serializer.end_array();\n"                                      \
            "}",                                                               \
            "msg", msg, "type", to_mcpack_typestr(cit, (field)), "lcfield",    \
...
#define TEMPLATE_INNER_SERIALIZE_REPEATED_INTEGRAL(msg, cit, printer, field, \
                                                   strict_cond)              \
    TEMPLATE_INNER_SERIALIZE_REPEATED(msg, cit, printer, field,              \
                                      ((cit) == IDL_AUTO || (strict_cond)),  \
                                      is_integral_type(cit))
...
static bool generate_serializing(const google::protobuf::Descriptor* d,
    ...
    for (int i = 0; i < d->field_count(); ++i) {
        ...
        if (cit == IDL_AUTO) {
            butil::string_printf(
                &comment_template, "// %s $type$ $name$ = $number$;\n",
                (f->is_repeated()
                     ? "repeated"
        ...
        impl.Print(comment_template.c_str(), "type", field_to_string(f), "name",
                   f->name(), "number",
                   butil::string_printf("%d", f->number()));
        if (f->is_repeated()) {
            switch (f->cpp_type()) {
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                TEMPLATE_SERIALIZE_REPEATED(
                    cit, impl, f, (cit == IDL_AUTO || cit == IDL_FLOAT),
                    (cit == IDL_DOUBLE));
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                TEMPLATE_SERIALIZE_REPEATED(
                    cit, impl, f, (cit == IDL_AUTO || cit == IDL_DOUBLE),
                    (cit == IDL_FLOAT));
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                if (f->type() ==
                    ...
                    TEMPLATE_SERIALIZE_REPEATED(
                        cit, impl, f, false,
                        (cit == IDL_AUTO || cit == IDL_STRING));
                } else if (f->type() ==
                           google::protobuf::FieldDescriptor::TYPE_BYTES) {
                    TEMPLATE_SERIALIZE_REPEATED(
                        cit, impl, f, false,
                        (cit == IDL_AUTO || cit == IDL_BINARY ||
                         cit == IDL_STRING));
                } else {
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                if (cit != IDL_AUTO) {
                    LOG(ERROR)
                        << "Disallow converting " << f->full_name() << " ("
                ...
                if (is_map_entry(msg2)) {
                    ...
                    impl.Print(
                        ...
                        "field", get_idl_name(f), "lcfield",
                        f->lowercase_name(), "msg2", cpp_name2);
                    const google::protobuf::FieldDescriptor* value_desc =
                        msg2->field(1);
                    switch (value_desc->cpp_type()) {
                    case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                        impl.Print(
                            "  serializer.add_int32(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                        impl.Print(
                            "  serializer.add_uint32(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                        impl.Print(
                    ...
                    case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
                        impl.Print(
                            "  serializer.add_int64(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                        impl.Print(
                            "  serializer.add_uint64(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
                        impl.Print(
                            "  serializer.add_bool(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                        impl.Print(
                            "  serializer.add_float(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                        impl.Print(
                            "  serializer.add_double(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                        impl.Print(
                            "  serializer.add_string(pair.key(), "
                            "pair.value());\n");
                        break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                        ...
                        impl.Print(
                            ...
                            "  serialize_$vmsg3$_body(pair.value(), "
                            "serializer, format);\n"
                ...
                impl.Print(
                    ...
                    "  if (msg.$lcfield$_size()) {\n"
                    "    serializer.begin_mcpack_array(\"$field$\", "
                    "::mcpack2pb::FIELD_OBJECT);\n"
                    "    for (int i = 0; i < msg.$lcfield$_size(); ++i) {\n"
                    "      serializer.begin_object();\n"
                    "      serialize_$vmsg2$_body(msg.$lcfield$(i), "
                    "serializer, format);\n"
                    "      serializer.end_object();\n"
                    "    }\n"
                    "    serializer.end_array();\n"
                    "  }",
                    "field", get_idl_name(f), "lcfield", f->lowercase_name(),
                    "vmsg2", var_name2);
                ...
                for (int j = 0; j < msg2->field_count(); ++j) {
                    const google::protobuf::FieldDescriptor* f2 =
                        msg2->field(j);
                    ConvertibleIdlType cit2 =
                        f2->options().GetExtension(idl_type);
                    impl.Print(
                        "serializer.begin_mcpack_array(\"$f2$\", "
                        "::mcpack2pb::FIELD_$TYPE$);\n"
                        "for (int i = 0; i < msg.$lcfield$_size(); ++i) {\n",
                        "f2", get_idl_name(f2), "TYPE",
                        (f2->is_repeated()
                             ? "ARRAY"
                             : to_mcpack_typestr_uppercase(cit2, (f2))),
                        "lcfield", f->lowercase_name());
                    impl.Indent();
                    if (f2->cpp_type() ==
                        ...
                        if (cit2 != IDL_AUTO) {
                            LOG(ERROR)
                                << "Disallow converting " << f2->full_name()
                                << " (" << f2->message_type()->full_name()
                                << ") to " << to_mcpack_typestr(cit2, f2)
                                << " (idl)";
                            return false;
                        }
                        std::string var_name3 = mcpack2pb::to_var_name(
                        ...
                        if (f2->is_repeated()) {
                            impl.Print(
                                "const int $lcfield2$_size = "
                                "msg.$lcfield$(i).$lcfield2$_size();\n"
                                "if ($lcfield2$_size) {\n"
                                "  "
                                "serializer.begin_mcpack_array(::mcpack2pb::"
                                "FIELD_OBJECT);\n"
                                "  for (int j = 0; j < $lcfield2$_size; ++j) "
                                "{\n"
                                "    serializer.begin_object();\n"
                                "    "
                                "serialize_$vmsg3$_body(msg.$lcfield$(i).$"
                                "lcfield2$(j), serializer, format);\n"
                                "    serializer.end_object();\n"
                                "  }\n"
                                "  serializer.end_array();\n"
                                "}",
                                "vmsg3", var_name3, "lcfield",
                                f->lowercase_name(), "lcfield2",
                                f2->lowercase_name());
                        ...
                        case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                        case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                            TEMPLATE_INNER_SERIALIZE_INTEGRAL(msgstr.c_str(),
                                                              cit2, impl, f2);
                            break;
                        case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                ...
                impl.Print(
                    "if (msg.has_$lcfield$()) {\n"
                    "  serializer.begin_object(\"$field$\");\n"
                    "  serialize_$vmsg$_body(msg.$lcfield$(), serializer, "
                    "format);\n"
                    "  serializer.end_object();\n"
                    "}",
                    "field", get_idl_name(f), "lcfield", f->lowercase_name(),
...
static bool generate_registration(const google::protobuf::FileDescriptor* file,
                                  google::protobuf::io::Printer& impl) {
    ...
    for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        impl.Print(
            ...
            "CHECK_EQ(0, g_$vmsg$_fields->init(std::max($field_count$, 1), "
            "30));\n",
            "vmsg", var_name, "field_count",
            ::butil::string_printf("%d", d->field_count()));
        for (int i = 0; i < d->field_count(); ++i) {
            const google::protobuf::FieldDescriptor* f = d->field(i);
            impl.Print(
                "(*g_$vmsg$_fields)[\"$field$\"] = ::set_$vmsg$_$lcfield$;\n",
                "vmsg", var_name, "field", get_idl_name(f), "lcfield",
                f->lowercase_name());
        }
        impl.Print(
            ...
            "};\n"
            "::mcpack2pb::register_message_handler_or_die(\"$fmsg$\", "
            "$vmsg$_handler);\n",
            "fmsg", d->full_name(), "vmsg", var_name);
    }
    impl.Outdent();
    impl.Outdent();
    impl.Print(
...
bool McpackToProtobuf::Generate(
    ...
    for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        if (!generate_parsing(d, ref_msgs, ref_maps, gimpl_printer)) {
            ::butil::string_printf(error,
                                   "Fail to generate parsing code for %s",
                                   d->full_name().c_str());
            return false;
        }
        if (!generate_serializing(d, ref_msgs, ref_maps, gimpl_printer)) {
            ::butil::string_printf(error,
                                   "Fail to generate serializing code for %s",
                                   d->full_name().c_str());
            return false;
        }
        std::string var_name = mcpack2pb::to_var_name(d->full_name());
        gdecl_printer.Print("::mcpack2pb::FieldMap* g_$vmsg$_fields = NULL;\n",
                            "vmsg", var_name);
    }
    if (!generate_declarations(ref_msgs, ref_maps, gdecl_printer)) {
        ::butil::string_printf(error, "Fail to generate declarations for %s",
                               cpp_name.c_str());
        return false;
    }
    if (!generate_registration(file, gimpl_printer)) {
        ::butil::string_printf(error,
                               "Fail to generate registration code for %s",
                               cpp_name.c_str());
        return false;
    }
    return true;
