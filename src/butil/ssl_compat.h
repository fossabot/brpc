...
BRPC_INLINE int RSA_set0_crt_params(RSA *r, BIGNUM *dmp1, BIGNUM *dmq1,
                                    BIGNUM *iqmp) {
...
BRPC_INLINE void RSA_get0_key(const RSA *r, const BIGNUM **n, const BIGNUM **e,
                              const BIGNUM **d) {
    if (n != NULL) *n = r->n;
    if (e != NULL) *e = r->e;
    if (d != NULL) *d = r->d;
...
BRPC_INLINE void RSA_get0_factors(const RSA *r, const BIGNUM **p,
                                  const BIGNUM **q) {
    if (p != NULL) *p = r->p;
    if (q != NULL) *q = r->q;
}

BRPC_INLINE void RSA_get0_crt_params(const RSA *r, const BIGNUM **dmp1,
...
BRPC_INLINE void DSA_get0_pqg(const DSA *d, const BIGNUM **p, const BIGNUM **q,
                              const BIGNUM **g) {
    if (p != NULL) *p = d->p;
    if (q != NULL) *q = d->q;
    if (g != NULL) *g = d->g;
...
BRPC_INLINE void DSA_get0_key(const DSA *d, const BIGNUM **pub_key,
...
BRPC_INLINE void DSA_SIG_get0(const DSA_SIG *sig, const BIGNUM **pr,
                              const BIGNUM **ps) {
    if (pr != NULL) *pr = sig->r;
    if (ps != NULL) *ps = sig->s;
}
...
BRPC_INLINE void DH_get0_pqg(const DH *dh, const BIGNUM **p, const BIGNUM **q,
                             const BIGNUM **g) {
    if (p != NULL) *p = dh->p;
    if (q != NULL) *q = dh->q;
    if (g != NULL) *g = dh->g;
...
BRPC_INLINE void DH_get0_key(const DH *dh, const BIGNUM **pub_key,
                             const BIGNUM **priv_key) {
...
BRPC_INLINE int RSA_bits(const RSA *r) { return (BN_num_bits(r->n)); }
