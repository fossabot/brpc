...
inline int LinuxKernelCmpxchg(Atomic32 old_value, Atomic32 new_value,
...
inline Atomic32 NoBarrier_CompareAndSwap(volatile Atomic32* ptr,
    ...
    for (;;) {
        prev_value = *ptr;
        if (prev_value != old_value) return prev_value;
        if (!LinuxKernelCmpxchg(old_value, new_value, ptr)) return old_value;
    }
}
