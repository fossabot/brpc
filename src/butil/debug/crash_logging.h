...
public:
    ScopedCrashKey(const butil::StringPiece& key,
                   const butil::StringPiece& value);
    ~ScopedCrashKey();
...
struct BUTIL_EXPORT CrashKey {
    ...
    // be truncated. If the value is larger than the |chunk_max_length| passed
    // to InitCrashKeys() but less than this value, it will be split into
...
BUTIL_EXPORT void SetCrashKeyReportingFunctions(
    SetCrashKeyValueFuncT set_key_func, ClearCrashKeyValueFuncT clear_key_func);
...
BUTIL_EXPORT std::vector<std::string> ChunkCrashKeyValue(
    const CrashKey& crash_key, const butil::StringPiece& value,
    size_t chunk_max_length);
