...
NOINLINE void CorruptMemoryBlock(bool induce_crash) {
    // NOTE(sebmarchand): We intentionally corrupt a memory block here in order
    // to
    //     trigger an Address Sanitizer (ASAN) error report.
    static const int kArraySize = 5;
    int* array                  = new int[kArraySize];
    // Encapsulate the invalid memory access into a try-catch statement to
    // prevent this function from being instrumented. This way the underflow
    // won't be detected but the corruption will (as the allocator will still be
    // hooked).
    try {
...
#if defined(SYZYASAN)
void AsanCorruptHeapBlock() { CorruptMemoryBlock(false); }

void AsanCorruptHeap() { CorruptMemoryBlock(true); }
#endif  // SYZYASAN
