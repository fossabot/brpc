...
void SetCrashKeyValue(const butil::StringPiece& key,
    ...
    if (!g_set_key_func_ || !g_crash_keys_) return;
    ...
    for (size_t i = chunks.size();
         i < NumChunksForLength(crash_key->max_length); ++i) {
        g_clear_key_func_(
            butil::StringPrintf(kChunkFormatString, key.data(), i + 1));
    }
...
void ClearCrashKey(const butil::StringPiece& key) {
    if (!g_clear_key_func_ || !g_crash_keys_) return;
...
void SetCrashKeyFromAddresses(const butil::StringPiece& key,
    ...
    if (addresses && count) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            length += s.length() + 1;
            if (length > kBreakpadValueMax) break;
...
ScopedCrashKey::~ScopedCrashKey() { ClearCrashKey(key_); }
...
const CrashKey* LookupCrashKey(const butil::StringPiece& key) {
    if (!g_crash_keys_) return NULL;
    CrashKeyMap::const_iterator it = g_crash_keys_->find(key.as_string());
    if (it == g_crash_keys_->end()) return NULL;
    return &(it->second);
}

void SetCrashKeyReportingFunctions(SetCrashKeyValueFuncT set_key_func,
...
std::vector<std::string> ChunkCrashKeyValue(const CrashKey& crash_key,
    ...
    std::string value_string =
        value.substr(0, crash_key.max_length).as_string();
    std::vector<std::string> chunks;
