...
void DemangleSymbols(std::string* text) {
    ...
    while (search_from < text->size()) {
        ...
        if (status == 0) {  // Demangling is successful.
            ...
            // Next time, we'll start right after the demangled symbol we
            // inserted.
...
void OutputFrameId(intptr_t frame_id, BacktraceOutputHandler* handler) {
    // Max unsigned 64-bit number in decimal has 20 digits
    // (18446744073709551615). Hence, 30 digits should be more than enough to
    // represent it in decimal (including the null-terminator).
    char buf[30] = {'\0'};
...
void ProcessBacktrace(void* const* trace, size_t size,
    ...
    if (in_signal_handler == 0) {
        scoped_ptr<char*, FreeDeleter> trace_symbols(
...
void StackDumpSignalHandler(int signal, siginfo_t* info, void* void_context) {
    ...
    if (BeingDebugged()) BreakDebugger();
    ...
    for (size_t i = 0; i < ARRAYSIZE_UNSAFE(registers); i++) {
        ...
        internal::itoa_r(registers[i].value, buf, sizeof(buf), 16,
                         kRegisterPadding);
        PrintToStderr(buf);

        if ((i + 1) % 4 == 0) PrintToStderr("\n");
    }
    PrintToStderr("\n");
#endif
    ...
    len = static_cast<size_t>(snprintf(
        buf, sizeof(buf), "ax: %x, bx: %x, cx: %x, dx: %x\n",
        context->uc_mcontext->__ss.__eax, context->uc_mcontext->__ss.__ebx,
        context->uc_mcontext->__ss.__ecx, context->uc_mcontext->__ss.__edx));
    write(STDERR_FILENO, buf, std::min(len, sizeof(buf) - 1));

    len = static_cast<size_t>(snprintf(
        buf, sizeof(buf), "di: %x, si: %x, bp: %x, sp: %x, ss: %x, flags: %x\n",
        context->uc_mcontext->__ss.__edi, context->uc_mcontext->__ss.__esi,
        context->uc_mcontext->__ss.__ebp, context->uc_mcontext->__ss.__esp,
        context->uc_mcontext->__ss.__ss, context->uc_mcontext->__ss.__eflags));
    write(STDERR_FILENO, buf, std::min(len, sizeof(buf) - 1));

    len = static_cast<size_t>(snprintf(
        buf, sizeof(buf), "ip: %x, cs: %x, ds: %x, es: %x, fs: %x, gs: %x\n",
        context->uc_mcontext->__ss.__eip, context->uc_mcontext->__ss.__cs,
        context->uc_mcontext->__ss.__ds, context->uc_mcontext->__ss.__es,
        context->uc_mcontext->__ss.__fs, context->uc_mcontext->__ss.__gs));
    write(STDERR_FILENO, buf, std::min(len, sizeof(buf) - 1));
...
public:
    ...
    virtual void HandleOutput(const char* output) OVERRIDE { (*os_) << output; }
...
void WarmUpBacktrace() {
    ...
    //             at
    //             ./third_party/tcmalloc/chromium/src/debugallocation.cc:1161
...
private:
    ...
    int GetFileDescriptor(const char* file_path) {
        ...
        if (file_path) {
            ...
            // int> using a const_iterator does not allocate dynamic memory,
            // hense it is async-signal-safe.
            std::map<std::string, int>::const_iterator it;
            for (it = modules_.begin(); it != modules_.end(); ++it) {
                if (strcmp((it->first).c_str(), file_path) == 0) {
                    // POSIX.1-2004 requires an implementation to guarantee that
                    // dup() is async-signal-safe.
                    fd = dup(it->second);
                    break;
                }
            }
    ...
    static int OpenObjectFileContainingPc(uint64_t pc, uint64_t& start_address,
        ...
        // std::vector<MappedMemoryRegion> using a const_iterator does not
        // allocate dynamic memory, hence it is async-signal-safe.
    ...
    // Parses /proc/self/maps in order to compile a list of all object file
    // names for the modules that are loaded in the current process. Returns
    // true on success.
...
#define O_CLOEXEC 02000000
    ...
    void OpenSymbolFiles() {
        ...
        // not considered safe for retail builds.  Hence it is only done in
        // debug builds.  For more details, take a look at:
        // http://crbug.com/341966 Enabling this to release mode would require
...
StackTrace::StackTrace() {
    ...
    count_ =
        butil::saturated_cast<size_t>(backtrace(trace_, arraysize(trace_)));
#else
    count_ = 0;
#endif
}
