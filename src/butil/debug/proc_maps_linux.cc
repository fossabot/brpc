...
bool ReadProcMaps(std::string* proc_maps) {
    // seq_file only writes out a page-sized amount on each call. Refer to
    ...
    while (true) {
        // To avoid a copy, resize |proc_maps| so read() can write directly into
        // it. Compute |buffer| afterwards since resize() may reallocate.
        ...
        // seq_file gets confused and the next call to read() will return
        // duplicate entries including the gate VMA again.
        //
...
bool ParseProcMaps(const std::string& input,
    ...
    // This isn't async safe nor terribly efficient, but it doesn't need to be
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        //
        // The final %n term captures the offset in the input string, which is
        // used to determine the path name. It *does not* increment the return
        // value. Refer to man 3 sscanf for details.
