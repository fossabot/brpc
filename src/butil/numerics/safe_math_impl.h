...
template <typename T>
typename enable_if<std::numeric_limits<T>::is_integer, T>::type CheckedAdd(
    T x, T y, RangeConstraint* validity) {
...
template <typename T>
typename enable_if<std::numeric_limits<T>::is_integer, T>::type CheckedSub(
    T x, T y, RangeConstraint* validity) {
    // Since the value of x+y is undefined if we have a signed type, we compute
    // it using the unsigned type of the same size.
    typedef typename UnsignedIntegerForSize<T>::type UnsignedDst;
    ...
    UnsignedDst uy      = static_cast<UnsignedDst>(y);
    UnsignedDst uresult = ux - uy;
    // Subtraction is valid if either x and y have same sign, or (x-y) and x
    // have the same sign.
    if (std::numeric_limits<T>::is_signed) {
...
CheckedMul(T x, T y, RangeConstraint* validity) {
    ...
    if (!(x || y)) {
        ...
        if (y > 0)
            *validity = x <= std::numeric_limits<T>::max() / y ? RANGE_VALID
                                                               : RANGE_OVERFLOW;
        else
        ...
        if (y > 0)
            ...
            *validity = y >= std::numeric_limits<T>::max() / x ? RANGE_VALID
                                                               : RANGE_OVERFLOW;
    }
...
public:
    ...
    explicit CheckedNumericState(
        Src value, typename enable_if<std::numeric_limits<Src>::is_specialized,
                                      int>::type = 0)
...
public:
    ...
    template <typename Src>
    CheckedNumericState(Src value, RangeConstraint validity,
                        typename enable_if<std::numeric_limits<Src>::is_integer,
                                           int>::type = 0) {
