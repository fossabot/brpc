...
public:
    ...
    // IsValid() is the public API to test if a CheckedNumeric is currently
    // valid.
    ...
    // ValueFloating() - Since floating point values include their validity
    // state, we provide an easy method for extracting them directly, without a
    // risk of crashing on a CHECK.
    T ValueFloating() const {
        COMPILE_ASSERT(std::numeric_limits<T>::is_iec559,
                       argument_must_be_float);
        return CheckedNumeric<T>::cast(*this).ValueUnsafe();
    }

    // validity() - DO NOT USE THIS IN EXTERNAL CODE - It is public right now
    // for tests and to avoid a big matrix of friend operator overloads. But the
    // values it returns are likely to change in the future.
    // Returns: current validity state (i.e. valid, overflow, underflow, nan).
    // TODO(jschuh): crbug.com/332611 Figure out and implement semantics for
    // saturation/wrapping so we can expose this state consistently and
    // implement saturated arithmetic.
    ...
    CheckedNumeric operator-() const {
        ...
        if (std::numeric_limits<T>::is_iec559) return CheckedNumeric<T>(value);
    ...
    static CheckedNumeric<T> cast(
        Src u, typename enable_if<std::numeric_limits<Src>::is_specialized,
                                  int>::type = 0) {
        return u;
    }
...
#define BUTIL_NUMERIC_ARITHMETIC_OPERATORS(NAME, OP, COMPOUND_OP)             \
    ...
    CheckedNumeric<T>& CheckedNumeric<T>::operator COMPOUND_OP(Src rhs) {     \
        *this =                                                               \
            CheckedNumeric<T>::cast(*this) OP CheckedNumeric<Src>::cast(rhs); \
        return *this;                                                         \
    }                                                                         \
