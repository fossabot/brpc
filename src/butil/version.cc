...
bool ParseVersionNumbers(const std::string& version_str,
    ...
    for (std::vector<std::string>::const_iterator it = numbers.begin();
         it != numbers.end(); ++it) {
        int num;
        if (!StringToInt(*it, &num)) return false;
        ...
        if (IntToString(num) != *it) return false;
...
int Version::CompareToWildcardString(const std::string& wildcard_string) const {
    ...
    // version is still smaller. Same logic for equality (e.g. comparing 1.2.2
    // to 1.2.2.* is 0 regardless of the wildcard). Under this logic,
