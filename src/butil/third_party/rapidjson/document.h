...
public:
    ...
    typedef
        typename Encoding::Ch Ch;  //!< Character type derived from Encoding.
    ...
    typedef const GenericValue*
        ConstValueIterator;  //!< Constant value iterator for iterating in
                             //!< array.
    ...
    GenericValue& AddMember(GenericValue& name, GenericValue& value,
        ...
        if (o.size >= o.capacity) {
            if (o.capacity == 0) {
                ...
                o.members = reinterpret_cast<Member*>(
                    allocator.Realloc(o.members, oldCapacity * sizeof(Member),
    ...
    */
    ...
    bool Accept(Handler& handler) const {
        ...
        default:
            ...
            if (IsInt())
                return handler.AddInt(data_.n.i.i);
            else if (IsUint())
                return handler.AddUint(data_.n.u.u);
            else if (IsInt64())
                return handler.AddInt64(data_.n.i64);
            else if (IsUint64())
                return handler.AddUint64(data_.n.u64);
            else
...
private:
    ...
    // to store 11-chars strings in 32-bit mode and 15-chars strings in 64-bit
    // mode inline (for `UTF8`-encoded strings).
    struct ShortString {
        enum {
    ...
    };  // at most as many bytes as "String" above => 12 bytes in 32-bit mode,
        // 16 bytes in 64-bit mode
    ...
    void SetArrayRaw(GenericValue* values, SizeType count,
        ...
        if (count) {
            data_.a.elements =
                (GenericValue*)allocator.Malloc(count * sizeof(GenericValue));
            std::memcpy(static_cast<void*>(data_.a.elements), values,
    ...
    void SetObjectRaw(Member* members, SizeType count, Allocator& allocator) {
        ...
        if (count) {
            ...
            std::memcpy(static_cast<void*>(data_.o.members), members,
                        count * sizeof(Member));
        } else
    ...
    */
    ...
    GenericDocument& ParseStream(InputStream& is) {
        ...
        if (parseResult_) {
            ...
            this->RawAssign(*stack_.template Pop<ValueType>(
                1));  // Add this-> to prevent issue 13.
        }
        return *this;
    }
