    ...
    *x++   = L;
    ...
    if (e < emin) {
        ...
        if (n >= nbits) {
            ...
            switch (rounding) {
            case Round_near:
                if (n == nbits && (n < 2 || any_on(b, n - 1))) goto ret_tiny;
                break;
            case Round_up:
                if (!sign) goto ret_tiny;
                break;
            case Round_down:
                if (sign) goto ret_tiny;
            }
...
#endif
            } else if (b->wds > k || ((n = nbits & kmask) != 0 &&
                                      hi0bits(x[k - 1]) < 32 - n)) {
                rshift(b, 1);
                if (++e > Emax) goto ovfl;
            }
...
have_i:
    ...
    i = dshift(d, d2);
    if ((b2 += i) > 0) b = lshift(b, b2);
    if ((d2 += i) > 0) d = lshift(d, d2);
...
if (nd > strtod_diglim) {
    ...
    if (nd < 9) { /* must recompute y */
        ...
        for (j = bc.dp1; i < nd; ++i) y = 10 * y + s0[j++] - '0';
    }
}
#endif
...
#ifndef NO_STRTOD_BIGCOMP
    if (bc.nd > nd && i <= 0) {
        if (bc.dsign) break; /* Must use bigcomp(). */
#ifdef Honor_FLT_ROUNDS
        if (bc.rounding != 1) {
            if (i < 0) break;
        } else
...
#endif
{
    ...
    j = sizeof(ULong);
    for (k = 0; sizeof(Bigint) - sizeof(ULong) - sizeof(int) + j <= (size_t)i;
         j <<= 1)
        k++;
    r  = (int *)Balloc(k);
    *r = k;
