...
public:
    ...
    // arrangement with "*" is invalid (e.g. 1.*.3 or 1.2.3*). This functions
    // defaults to standard Version behavior (IsValid) if no wildcard is
    // present.
    ...
    // not end in wildcard sequence ".*". IsValidWildcard(wildcard_string) must
    // be true before using this function.
