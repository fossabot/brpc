...
public:
    ...
    // Use line number as the primary key (because it is fast, and usually gets
    // us a difference), and then pointers as secondary keys (just to get some
    // distinctions).
    ...
    // string with HTML characters in the function names escaped, and append
...
struct BUTIL_EXPORT LocationSnapshot {
    // The default constructor is exposed to support the IPC serialization
    // macros.
    LocationSnapshot();
