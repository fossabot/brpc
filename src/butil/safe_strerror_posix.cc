...
wrap_posix_strerror_r(char *(*strerror_r_ptr)(int, char *, size_t), int err,
    ...
    // The GNU version never fails. Unknown errors get an "unknown error"
    // message. The result is always null terminated.
}
...
wrap_posix_strerror_r(int (*strerror_r_ptr)(int, char *, size_t), int err,
    ...
    // (but in such a scenario this function is never called). Sadly we can't
    // use C++-style casts because the appropriate one is reinterpret_cast but
    // it's considered illegal to reinterpret_cast a type to itself, so we get
    // an error in the opposite case.
    ...
    if (result == 0) {
        ...
        // Error. POSIX is vague about whether the return value is itself a
        // system error code or something else. On Linux currently it is -1 and
        // errno is set. On BSD-derived systems it is a system error and errno
        // is unchanged. We try and detect which case it is so as to put as much
        // useful info as we can into our message.
        ...
        if (new_errno != old_errno) {
            ...
            // something else that doesn't provide any info, and errno is the
            // error.
            strerror_error = new_errno;
        } else {
            // Either the error from strerror_r was the same as the previous
            // value, or errno wasn't used. Assume the latter.
