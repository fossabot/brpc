...
//
//   butil::SmallMap<std::map<MyKey, MyValue>, 4, std::equal_to<KyKey> >
//
//
// USAGE
// -----
...
//
// example:
//   butil::SmallMap< std::map<string, int> > days;
//   days["sunday"   ] = 0;
//   days["monday"   ] = 1;
//   days["tuesday"  ] = 2;
//   days["wednesday"] = 3;
...
struct has_key_equal {
    typedef char
        sml;  // "small" is sometimes #defined so we use an abbreviation.
    typedef struct {
        char dummy[2];
    } big;
    // Two functions, one accepts types that have a key_equal member, and one
    // that accepts anything. They each return a value of a different size, so
    // we can determine at compile-time which function would have been called.
    template <typename U>
...
struct select_equal_key {
    struct equal_key {
        bool operator()(const typename M::key_type& left,
            ...
            // Implements equality in terms of a strict weak ordering
            // comparator.
...
public:
    ...
    public:
        typedef
            typename NormalMap::iterator::iterator_category iterator_category;
    ...
    public:
        ...
        typedef
            typename NormalMap::const_iterator::difference_type difference_type;
        ...
        // Non-explicit ctor lets us convert regular iterators to const
        // iterators
    ...
    // representation.
    bool UsingFullMap() const { return size_ < 0; }
...
template <typename NormalMap, int kArraySize, typename EqualKey,
          typename Functor>
inline bool SmallMap<NormalMap, kArraySize, EqualKey, Functor>::iterator::
