...
private:
    ...
    typedef typename MapType<KeyType, typename PayloadList::iterator>::Type
        KeyIndex;
...
public:
    ...
    // The max_size is the size at which the cache will prune its members to
    // when a new item is inserted. If the caller wants to manager this itself
    // (for example, maybe it has special work to do when something is evicted),
    // it can pass NO_AUTO_EVICT to not restrict the cache size.
    ...
    virtual ~MRUCacheBase() {
        iterator i = begin();
        while (i != end()) i = Erase(i);
    }

    size_type max_size() const { return max_size_; }

    // Inserts a payload item with the given key. If an existing item has
    // the same key, it is removed prior to insertion. An iterator indicating
    // the inserted item will be returned (this will always be the front of the
    // list).
    //
    // The payload will be copied. In the case of an OwningMRUCache, this
    ...
    iterator Put(const KeyType& key, const PayloadType& payload) {
        ...
        if (index_iter != index_.end()) {
            // Erase the reference to it. This will call the deletor on the
            // removed element. The index reference will be replaced in the code
            // below.
            Erase(index_iter->second);
        } else if (max_size_ != NO_AUTO_EVICT) {
            // New item is being inserted which might make it larger than the
            // maximum size: kick the oldest thing out if necessary.
    ...
    // Retrieves the contents of the given key, or end() if not found. This
    // method has the side effect of moving the requested item to the front of
    // the recency list.
    //
    ...
    reverse_iterator Erase(reverse_iterator pos) {
        ...
        // the forward iterator that base() returns is actually one past the
    ...
    void ShrinkToSize(size_type new_size) {
        for (size_type i = size(); i > new_size; i--) Erase(rbegin());
    }
...
class MRUCache : public MRUCacheBase<KeyType, PayloadType,
...
class OwningMRUCache
    : public MRUCacheBase<KeyType, PayloadType,
...
struct MRUCacheHashMap {
    typedef butil::hash_map<KeyType, ValueType> Type;
};

// This class is similar to MRUCache, except that it uses butil::hash_map as
// the map type instead of std::map. Note that your KeyType must be hashable
// to use this cache.
template <class KeyType, class PayloadType>
class HashingMRUCache
    : public MRUCacheBase<KeyType, PayloadType,
                          MRUCacheNullDeletor<PayloadType>, MRUCacheHashMap> {
private:
    typedef MRUCacheBase<KeyType, PayloadType, MRUCacheNullDeletor<PayloadType>,
                         MRUCacheHashMap>
        ParentType;
