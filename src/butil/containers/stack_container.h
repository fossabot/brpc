...
public:
    ...
    struct Source {
        ...
        butil::AlignedMemory<sizeof(T[stack_capacity]), ALIGNOF(T)>
            stack_buffer_;
    ...
    // Actually do the allocation. Use the stack buffer if nobody has used it
    // yet and the size requested fits. Otherwise, fall through to the standard
    // allocator.
...
public:
    ...
    StackContainer() : allocator_(&stack_data_), container_(allocator_) {
        // Make the container use the stack allocation by reserving our buffer
    ...
    // Support operator-> to get to the container. This allows nicer syntax
    // like:
    //   StackContainer<...> foo;
...
class StackString16
    : public StackContainer<
          std::basic_string<char16, butil::string16_char_traits,
