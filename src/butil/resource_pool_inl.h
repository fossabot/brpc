...
public:
    static const size_t value = (N2 > ResourcePoolBlockMaxItem<T>::value
                                     ? ResourcePoolBlockMaxItem<T>::value
                                     : N2);
};
...
#define BAIDU_RESOURCE_POOL_GET(CTOR_ARGS)                                       \
    ...
    return NULL;
        ...
        inline T* get(ResourceId<T>* id) { BAIDU_RESOURCE_POOL_GET(); }
    ...
    ResourcePoolInfo describe_resources() const {
        ...
        for (size_t i = 0; i < info.block_group_num; ++i) {
            ...
            size_t nblock = std::min(
                bg->nblock.load(butil::memory_order_relaxed), RP_GROUP_NBLOCK);
...
private:
    ...
    ~ResourcePool() { pthread_mutex_destroy(&_free_chunks_mutex); }
...
template <typename T>
BAIDU_THREAD_LOCAL
    typename ResourcePool<T>::LocalPool* ResourcePool<T>::_local_pool = NULL;
...
template <typename T>
butil::static_atomic<long> ResourcePool<T>::_nlocal =
    BUTIL_STATIC_ATOMIC_INIT(0);

template <typename T>
butil::static_atomic<size_t> ResourcePool<T>::_ngroup =
    BUTIL_STATIC_ATOMIC_INIT(0);
...
#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
template <typename T>
butil::static_atomic<size_t> ResourcePool<T>::_global_nfree =
    BUTIL_STATIC_ATOMIC_INIT(0);
#endif
