...
int DescribeCustomizedErrno(int error_code, const char* error_name,
    ...
    if (desc) {
        ...
        {
            fprintf(stderr,
                    "Fail to define %s(%d) which is already defined as `%s', "
                    "abort.",
...
const char* berror(int error_code) {
...
#if defined(OS_MACOSX)
        const int rc =
            strerror_r(error_code, butil::tls_error_buf, butil::ERROR_BUFSIZE);
    ...
    snprintf(butil::tls_error_buf, butil::ERROR_BUFSIZE, "Unknown error %d",
             error_code);
    return butil::tls_error_buf;
}

const char* berror() { return berror(errno); }
