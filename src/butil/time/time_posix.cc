...
butil::TimeTicks ClockNow(clockid_t clk_id) {
    ...
    absolute_micro = (static_cast<int64_t>(ts.tv_sec) *
...
Time Time::Now() {
    ...
    if (gettimeofday(&tv, &tz) != 0) {
        ...
        // Return null instead of uninitialized |tv| value, which contains
        // random garbage data. This may result in the crash seen in
        // crbug.com/147570.
        return Time();
    }
    // Combine seconds and microseconds in a 64-bit field containing
    // microseconds since the epoch.  That's enough for nearly 600 centuries.
...
Time Time::FromExploded(bool is_local, const Exploded& exploded) {
    ...
    if (seconds == -1) {
        // Get the time values with tm_isdst == 0 and 1, then select the closest
        // one to UTC 00:00:00 that isn't -1.
    ...
    // return is the best that can be done here.  It's not ideal, but it's
    // better than failing here or ignoring the overflow case and treating each
    // time overflow as one second prior to the epoch.
    if (seconds == -1 && (exploded.year < 1969 || exploded.year > 1970)) {
        ...
        // handle time zone and DST offsets.)  Otherwise, return the most future
        // or past time representable.  Assumes the time_t epoch is 1970-01-01
        // 00:00:00 UTC.
        //
        ...
        // allow for proper round-tripping between exploded and counter-type
        // time representations in the presence of possible truncation to time_t
        // by division and use with other functions that accept time_t.
        //
        // When representing the most distant time in the future, add in an
        // extra 999ms to avoid the time being less than any other possible
...
// TimeTicks ------------------------------------------------------------------
// static
TimeTicks TimeTicks::Now() { return ClockNow(CLOCK_MONOTONIC); }
