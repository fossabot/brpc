...
Time Time::FromTimeT(time_t tt) {
    ...
    if (tt == std::numeric_limits<time_t>::max()) return Max();
...
time_t Time::ToTimeT() const {
    ...
    if (std::numeric_limits<int64_t>::max() - kTimeTToMicrosecondsOffset <=
        us_) {
...
Time Time::FromDoubleT(double dt) {
    ...
    if (dt == std::numeric_limits<double>::infinity()) return Max();
    return Time(static_cast<int64_t>(
        (dt * static_cast<double>(kMicrosecondsPerSecond)) +
...
Time Time::FromJsTime(double ms_since_epoch) {
    ...
    // 0 as the null time.
    if (ms_since_epoch == std::numeric_limits<double>::infinity()) return Max();
    return Time(
...
// static
bool Time::FromStringInternal(const char* time_string, bool is_local,
                              Time* parsed_time) {
