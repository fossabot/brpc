...
public:
    ...
    // Returns the maximum time delta, which should be greater than any
    // reasonable time delta we might compare it to. Adding or subtracting the
    // maximum time delta to a time or another time delta has an undefined
    // result.
    static TimeDelta Max();

    // Returns the internal numeric value of the TimeDelta object. Please don't
    // use this and do arithmetic on it, as it is more error prone than using
    // the provided operators. For serializing, use FromInternalValue to
    // reconstitute.
...
public:
    ...
    // this global header and put in the platform-specific ones when we remove
    // the migration code.
    ...
    // Returns the maximum time, which should be greater than any reasonable
    // time with which we might compare it.
    static Time Max();
    ...
    // Activates or deactivates the high resolution timer based on the
    // |activate| flag.  If the HighResolutionTimer is not Enabled (see
...
private:
    ...
    static bool FromStringInternal(const char* time_string, bool is_local,
                                   Time* parsed_time);
...
inline TimeDelta TimeDelta::FromDays(int days) {
    ...
    if (days == std::numeric_limits<int>::max()) return Max();
...
inline TimeDelta TimeDelta::FromHours(int hours) {
    // Preserve max to prevent overflow.
    if (hours == std::numeric_limits<int>::max()) return Max();
...
inline TimeDelta TimeDelta::FromMinutes(int minutes) {
    // Preserve max to prevent overflow.
    if (minutes == std::numeric_limits<int>::max()) return Max();
...
inline TimeDelta TimeDelta::FromSeconds(int64_t secs) {
    // Preserve max to prevent overflow.
    if (secs == std::numeric_limits<int64_t>::max()) return Max();
...
inline TimeDelta TimeDelta::FromMilliseconds(int64_t ms) {
    // Preserve max to prevent overflow.
    if (ms == std::numeric_limits<int64_t>::max()) return Max();
...
inline TimeDelta TimeDelta::FromSecondsD(double secs) {
    // Preserve max to prevent overflow.
    if (secs == std::numeric_limits<double>::infinity()) return Max();
...
inline TimeDelta TimeDelta::FromMillisecondsD(double ms) {
    // Preserve max to prevent overflow.
    if (ms == std::numeric_limits<double>::infinity()) return Max();
...
inline TimeDelta TimeDelta::FromMicroseconds(int64_t us) {
    // Preserve max to prevent overflow.
    if (us == std::numeric_limits<int64_t>::max()) return Max();
    return TimeDelta(us);
}
...
inline TimeDelta TimeDelta::FromMicrosecondsD(double us) {
    // Preserve max to prevent overflow.
    if (us == std::numeric_limits<double>::infinity()) return Max();
...
public:
    ...
    static bool IsThreadNowSupported() { return false; }

    // Returns thread-specific CPU-time on systems that support this feature.
    // Needs to be guarded with a call to IsThreadNowSupported(). Use this timer
    // to (approximately) measure how much time the calling thread spent doing
    // actual work vs. being de-scheduled. May return bogus results if the
    // thread migrates to another CPU between two calls.
    ...
    // calls to TimeTicks::Now() will have the higher resolution provided by
    // QPC. Returns true if high resolution time was successfully enabled.
    ...
    // Note: Upon first invocation, this function takes a snapshot of the
