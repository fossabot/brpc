...
public:
    ...
    // ---------------------------------------------------------------------------
    // These waiters are always stack allocated and don't delete themselves.
    // Thus there's no problem and the ABA tag is the same as the object
    // pointer.
    // ---------------------------------------------------------------------------
    virtual bool Compare(void* tag) OVERRIDE { return this == tag; }
    ...
    butil::Lock* lock() { return &lock_; }

    butil::ConditionVariable* cv() { return &cv_; }
...
bool WaitableEvent::TimedWait(const TimeDelta& max_time) {
    ...
    Enqueue(&sw);
    kernel_->lock_.Release();
    // We are violating locking order here by holding the SyncWaiter lock but
    // not the WaitableEvent lock. However, this is safe because we don't lock
    // @lock_ again before unlocking it.
    ...
    for (;;) {
        ...
        if (sw.fired() || (finite_time && current_time >= end_time)) {
            ...
            // We can't acquire @lock_ before releasing the SyncWaiter lock
            // (because of locking order), however, in between the two a signal
            // could be fired and @sw would accept it, however we will still
            // return false, so the signal would be lost on an auto-reset
            // WaitableEvent. Thus we call Disable which makes sw::Fire return
            // false.
            sw.Disable();
            sw.lock()->Release();
...
size_t WaitableEvent::WaitMany(WaitableEvent** raw_waitables, size_t count) {
    ...
    // the array of waitables by address. We actually sort a pairs so that we
    // can map back to the original index values later.
    ...
    if (r) {
        ...
        // enqueued anywhere. EnqueueMany returns the count of remaining
        // waitables when the signaled one was seen, so the index of the
        // signaled event is
        // @count - @r.
    ...
    // Take the locks of each WaitableEvent in turn (except the signaled one)
    // and remove our SyncWaiter from the wait-list
    for (size_t i = 0; i < count; ++i) {
        if (raw_waitables[i] != signaled_event) {
            raw_waitables[i]->kernel_->lock_.Acquire();
            // There's no possible ABA issue with the address of the SyncWaiter
            // here because it lives on the stack. Thus the tag value is just
