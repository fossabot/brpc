...
void OffsetAdjuster::AdjustOffsets(
    ...
    if (!offsets_for_adjustment || adjustments.empty()) return;
...
void OffsetAdjuster::AdjustOffset(const Adjustments& adjustments,
                                  size_t* offset) {
    if (*offset == string16::npos) return;
    int adjustment = 0;
    for (Adjustments::const_iterator i = adjustments.begin();
         i != adjustments.end(); ++i) {
        if (*offset <= i->original_offset) break;
...
void OffsetAdjuster::UnadjustOffsets(
    ...
    if (!offsets_for_unadjustment || adjustments.empty()) return;
...
void OffsetAdjuster::UnadjustOffset(const Adjustments& adjustments,
    ...
    for (Adjustments::const_iterator i = adjustments.begin();
         i != adjustments.end(); ++i) {
        if (*offset + adjustment <= i->original_offset) break;
        adjustment += static_cast<int>(i->original_length - i->output_length);
...
void OffsetAdjuster::MergeSequentialAdjustments(
    ...
    Adjustments::iterator adjusted_iter =
        adjustments_on_adjusted_string->begin();
    ...
    // done processing the current |adjusted_iter|; they are not yet reflected
    // in |shift|.
    size_t shift                = 0;
    ...
    while (adjusted_iter != adjustments_on_adjusted_string->end()) {
        if ((first_iter == first_adjustments.end()) ||
            ...
            // Entire |adjusted_iter| (accounting for its shift and including
            // its whole original length) comes before |first_iter|.
            //
            ...
            // |first_iter| comes before the |adjusted_iter| (as adjusted by
            // |shift|).

            // It's not possible for the adjustments to overlap.  (It shouldn't
            // be possible that we have an |adjusted_iter->original_offset|
            // that, when adjusted by the computed |shift|, is in the middle of
            ...
            // Add the |first_adjustment_iter| to the full set of adjustments
            // while making sure |adjusted_iter| continues pointing to the same
            // element. We do this by inserting the |first_adjustment_iter|
            // right before |adjusted_iter|, then incrementing |adjusted_iter|
            // so it points to the following element.
            ...
            // The first adjustment adjusted something that then got further
            // adjusted by the second set of adjustments.  In other words,
            // |first_iter| points to something in the range covered by
            // |adjusted_iter|'s length (after accounting for |shift|).
            // Precisely,
            //   adjusted_iter->original_offset + shift
            //   <=
            //   first_iter->original_offset
            ...
            // happened in |first_iter|, then advance to the next
            // |first_adjustments| because we dealt with the current one.
            const int collapse = static_cast<int>(first_iter->original_length) -
                                 static_cast<int>(first_iter->output_length);
            // This function does not know how to deal with a string that
            // expands and then gets modified, only strings that collapse and
            // then get modified.
    ...
    if (first_iter != first_adjustments.end()) {
        ...
        // (Their offsets are already correct with respect to the original
        // string.) Append them all.
...
bool ConvertUnicode(const SrcChar* src, size_t src_len, DestStdString* output,
                    OffsetAdjuster::Adjustments* adjustments) {
    if (adjustments) adjustments->clear();
...
bool UTF8ToUTF16WithAdjustments(
    const char* src, size_t src_len, string16* output,
...
string16 UTF8ToUTF16WithAdjustments(
    ...
    butil::OffsetAdjuster::Adjustments* adjustments) {
    string16 result;
    UTF8ToUTF16WithAdjustments(utf8.data(), utf8.length(), &result,
                               adjustments);
    return result;
}
...
string16 UTF8ToUTF16AndAdjustOffsets(
    ...
    std::for_each(offsets_for_adjustment->begin(),
                  ...
                  LimitOffset<butil::StringPiece>(utf8.length()));
    OffsetAdjuster::Adjustments adjustments;
...
std::string UTF16ToUTF8AndAdjustOffsets(
    ...
    std::for_each(offsets_for_adjustment->begin(),
                  offsets_for_adjustment->end(),
                  LimitOffset<butil::StringPiece16>(utf16.length()));
    std::string result;
