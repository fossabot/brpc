...
class StringPieceToNumberTraits
    : public BaseIteratorRangeToNumberTraits<StringPiece::const_iterator, VALUE,
                                             BASE> {};
...
bool String16ToIntImpl(const StringPiece16& input, VALUE* output) {
    return IteratorRangeToNumber<
        StringPiece16ToNumberTraits<VALUE, 10> >::Invoke(input.begin(),
                                                         input.end(), output);
}
...
std::string IntToString(int value) {
    return IntToStringT<std::string, int, unsigned int, true>::IntToString(
        value);
}
...
string16 UintToString16(unsigned int value) {
    return IntToStringT<string16, unsigned int, unsigned int,
                        false>::IntToString(value);
}

std::string Int64ToString(int64_t value) {
    return IntToStringT<std::string, int64_t, uint64_t, true>::IntToString(
        value);
}
...
string16 Uint64ToString16(uint64_t value) {
    return IntToStringT<string16, uint64_t, uint64_t, false>::IntToString(
        value);
}
...
bool StringToDouble(const std::string& input, double* output) {
    ...
    *output      = dmg_fp::strtod(input.c_str(), &endptr);
    ...
    //    string does not begin with a parseable number.  endptr is compared to
    //    the expected end given the string's stated length to correctly catch
    //    cases where the string contains embedded NUL characters.
