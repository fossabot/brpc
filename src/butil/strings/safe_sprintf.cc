...
namespace internal {
void SetSafeSPrintfSSizeMaxForTest(size_t max) { kSSizeMax = max; }

size_t GetSafeSPrintfSSizeMaxForTest() { return kSSizeMax; }
}  // namespace internal
...
// constexpr until 4.6, which all don't mark numeric_limits::max() as constexp.
    ...
    ~Buffer() {
        // The code calling the constructor guaranteed that there was enough
        // space to store a trailing NUL -- and in debug builds, we are actually
        // verifying this with DEBUG_CHECK()s in the constructor. So, we can
        // always unconditionally write the NUL byte in the destructor.  We do
        // not need to adjust the count_, as SafeSPrintf() copies snprintf() in
        // not including the NUL byte in its return code.
    ...
    bool IToASCII(bool sign, bool upcase, int64_t i, int base, char pad,
                  size_t padding, const char* prefix);
...
bool Buffer::IToASCII(bool sign, bool upcase, int64_t i, int base, char pad,
                      size_t padding, const char* prefix) {
    ...
    // If padding with '0' zero, emit the prefix or '-' character now.
    // Otherwise, make the prefix accessible in reverse order, so that we can
    // later output it right between padding and the number. We cannot choose
    // the easier approach of just reversing the number, as that fails in
    // situations where we need to truncate numbers that have padding and/or
    // prefixes.
    ...
    do {
        ...
        if (count_ >= size_) {
            if (start < size_) {
                // It is rare that we need to output a partial number. But if
                // asked to do so, we will still make sure we output the correct
                // number of leading digits. Since we are generating the digits
                // in reverse order, we actually have to discard digits in the
                // order that we have already emitted them. This is essentially
                // equivalent to:
                //   memmove(buffer_ + start, buffer_ + start + 1, size_ - start
                //   - 1)
                ...
                // Need to increment either |count_| or |discarded| to make
                // progress. The latter is more efficient, as it eventually
                // triggers fast handling of padding. But we have to ensure we
                // don't accidentally change the overall state (i.e. switch the
                // state-machine from discarding to non-discarding). |count_|
                // needs to always stay bigger than |size_|.
                --count_;
                ++discarded;
            }
        }
        ...
        if (padding > 0) {
            ...
            // excessive padding, but our output buffer is limited in size. Even
            // if we output a 64bit number in binary, we would never write more
            // than 64 plus prefix non-padding characters. So, once this limit
            // has been passed, any further state change can be computed
            // arithmetically; we know that by this time, our entire final
            // output consists of padding characters that have all already been
            // output.
...
ssize_t SafeSNPrintf(char* buf, size_t sz, const char* fmt, const Arg* args,
    ...
    for (unsigned int cur_arg = 0; *fmt && !buffer.OutOfAddressableSpace();) {
        ...
        format_character_found:
            ...
            case '8':
            case '9':
                // Found a width parameter. Convert to an integer value and
                // store in "padding". If the leading digit is a zero, change
                // the padding character from a space ' ' to a zero '0'.
                pad = ch == '0' ? '0' : ' ';
                for (;;) {
                    // The maximum allowed padding fills all the available
                    // address space and leaves just enough space to insert the
                    // trailing NUL.
                    const size_t max_padding = kSSizeMax - 1;
                    if (padding > max_padding / 10 ||
                        10 * padding > max_padding - (ch - '0')) {
                        DEBUG_CHECK(
                            padding <= max_padding / 10 &&
                                10 * padding <= max_padding - (ch - '0'),
                            "");
                        // Integer overflow detected. Skip the rest of the width
                        // until we find the format character, then do the
                    ...
                    if (padding > max_padding) {
                        // This doesn't happen for "sane" values of kSSizeMax.
                        // But once kSSizeMax gets smaller than about 10, our
                        // earlier range checks are incomplete. Unittests do
                        // trigger this artificial corner case.
                        DEBUG_CHECK(padding <= max_padding, "");
                        goto padding_overflow;
                    }
                    ch = *fmt++;
                    if (ch < '0' || ch > '9') {
                        // Reached the end of the width parameter. This is where
                        // the format character is found.
            ...
            case 'p': {  // Output a pointer value.
                ...
                if (ch != 'p') {
                    ...
                    if (arg.type != Arg::INT && arg.type != Arg::UINT) {
                        DEBUG_CHECK(
                            arg.type == Arg::INT || arg.type == Arg::UINT, "");
                        goto fail_to_expand;
                    }
                    i = arg.i;

                    if (ch != 'd') {
                        // The Arg() constructor automatically performed sign
                        // expansion on signed parameters. This is great when
                        // outputting a %d decimal number, but can result in
                        // unexpected leading 0xFF bytes when outputting a %x
                        // hexadecimal number. Mask bits, if necessary. We have
                        // to do this here, instead of in the Arg() constructor,
                        // as the Arg() constructor cannot tell whether we will
                        // output a %d or a %x. Only the latter should
                        // experience masking.
                    ...
                    if (arg.type == Arg::POINTER) {
                        ...
                        i = reinterpret_cast<uintptr_t>(arg.str);
                               ...
                               arg.width == sizeof(NULL) &&
                               arg.i == 0) {  // Allow C++'s version of NULL
                        i = 0;
                    } else {
                        DEBUG_CHECK(
                            arg.type == Arg::POINTER || arg.type == Arg::STRING,
                            "");
                        goto fail_to_expand;
                    }
                ...
                // Use IToASCII() to convert to ASCII representation. For
                // decimal numbers, optionally print a sign. For hexadecimal
                // numbers, distinguish between upper and lower case. %p
                // addresses are always printed as upcase. Supports base 8, 10,
                // and 16. Prints padding and/or prefixes, if so requested.
                buffer.IToASCII(ch == 'd' && arg.type == Arg::INT, ch != 'x', i,
                                ch == 'o' ? 8 : ch == 'd' ? 10 : 16, pad,
                                padding, prefix);
                break;
            }
            case 's': {
                ...
                // Printing a string involves nothing more than copying it into
                // the output buffer and making sure we don't output more bytes
                // than available space; Out() takes care of doing that.
                for (const char* src = s; *src;) {
                    buffer.Out(*src++);
                }
                break;
            ...
            case '%':
                ...
                goto copy_verbatim;
            fail_to_expand:
                // C++ gives us tools to do type checking -- something that
                // snprintf() could never really do. So, whenever we see
                // arguments that don't match up with the format string, we
                // refuse to output them. But since we have to be extremely
                // conservative about being async- signal-safe, we are limited
                // in the type of error handling that we can do in production
                // builds (in debug builds we can use DEBUG_CHECK() and hope for
                // the best). So, all we do is pass the format string unchanged.
                // That should eventually get the user's attention; and in the
                // meantime, it hopefully doesn't lose too much data.
            default:
                // Unknown or unsupported format character. Just copy verbatim
                // to output.
                buffer.Out('%');
                DEBUG_CHECK(ch, "");
                if (!ch) {
