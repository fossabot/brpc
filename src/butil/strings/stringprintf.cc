...
static void StringAppendVT(StringType* dst,
    ...
    while (true) {
        if (result < 0) {
#if defined(OS_WIN)
            // On Windows, vsnprintfT always returns the number of characters in
            // a fully-formatted string, so if we reach this point, something
            // else is wrong and no amount of buffer-doubling is going to fix
            // it.
            return;
#else
        ...
        if (mem_length > 32 * 1024 * 1024) {
            ...
            // against huge allocations when using vsnprintfT implementations
            // that return -1 for reasons other than overflow without setting
            // errno.
            DLOG(WARNING)
        ...
        // NOTE: You can only use a va_list once.  Since we're in a while loop,
        // we need to make a new copy each time so we don't use up the original.
        GG_VA_COPY(ap_copy, ap);
