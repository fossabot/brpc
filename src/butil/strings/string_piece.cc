...
template <typename STR>
size_t rfindT(const BasicStringPiece<STR>& self, const BasicStringPiece<STR>& s,
              size_t pos) {
...
size_t find_first_of(const StringPiece& self, const StringPiece& s,
                     size_t pos) {
...
size_t find_first_of(const StringPiece16& self, const StringPiece16& s,
                     size_t pos) {
    StringPiece16::const_iterator found =
        std::find_first_of(self.begin() + pos, self.end(), s.begin(), s.end());
    if (found == self.end()) return StringPiece16::npos;
    return found - self.begin();
}

// 8-bit version using lookup table.
size_t find_first_not_of(const StringPiece& self, const StringPiece& s,
                         size_t pos) {
...
BUTIL_EXPORT size_t find_first_not_of(const StringPiece16& self,
    ...
    if (self.size() == 0) return StringPiece16::npos;
...
// 16-bit brute-force version.
size_t find_last_of(const StringPiece16& self, const StringPiece16& s,
                    size_t pos) {
    if (self.size() == 0) return StringPiece16::npos;

    for (size_t self_i = std::min(pos, self.size() - 1);; --self_i) {
        for (size_t s_i = 0; s_i < s.size(); s_i++) {
            if (self.data()[self_i] == s[s_i]) return self_i;
        }
        if (self_i == 0) break;
    }
    return StringPiece16::npos;
...
// 8-bit version using lookup table.
size_t find_last_not_of(const StringPiece& self, const StringPiece& s,
                        size_t pos) {
    if (self.size() == 0) return StringPiece::npos;
...
// 16-bit brute-force version.
size_t find_last_not_of(const StringPiece16& self, const StringPiece16& s,
                        size_t pos) {
    if (self.size() == 0) return StringPiece::npos;
...
template <typename STR>
BasicStringPiece<STR> substrT(const BasicStringPiece<STR>& self, size_t pos,
                              size_t n) {
