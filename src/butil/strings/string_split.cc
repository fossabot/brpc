...
void SplitStringT(const STR& str, const typename STR::value_type s,
    ...
    for (size_t i = 0; i <= c; ++i) {
        if (i == c || str[i] == s) {
            ...
            if (trim_whitespace) TrimWhitespace(tmp, TRIM_ALL, &tmp);
            // Avoid converting an empty or all-whitespace source string into a
            // vector of one empty string.
            if (i != c || !r->empty() || !tmp.empty()) r->push_back(tmp);
            last = i + 1;
        }
    }
}
...
template <typename STR>
void SplitStringUsingSubstrT(const STR& str, const STR& s,
                             std::vector<STR>* r) {
    r->clear();
...
void SplitString(const string16& str, char16 c, std::vector<string16>* r) {
    DCHECK(CBU16_IS_SINGLE(c));
    SplitStringT(str, c, true, r);
}

void SplitString(const butil::StringPiece16& str, char16 c,
...
void SplitString(const StringPiece& str, char c, std::vector<StringPiece>* r) {
#if CHAR_MIN < 0
    DCHECK(c >= 0);
#endif
    DCHECK(c < 0x7F);
...
template <typename STR>
bool SplitStringIntoKeyValuePairsT(
    const STR& line, char key_value_delimiter, char key_value_pair_delimiter,
    ...
    for (size_t i = 0; i < pairs.size(); ++i) {
        ...
        STR key;
        STR value;
        if (!SplitStringIntoKeyValueT(pairs[i], key_value_delimiter, &key,
                                      &value)) {
...
bool SplitStringIntoKeyValuePairs(const std::string& line,
    ...
    return SplitStringIntoKeyValuePairsT(
        line, key_value_delimiter, key_value_pair_delimiter, key_value_pairs);
}
...
void SplitStringUsingSubstr(const string16& str, const string16& s,
...
void SplitStringDontTrim(const string16& str, char16 c,
                         std::vector<string16>* r) {
    DCHECK(CBU16_IS_SINGLE(c));
    SplitStringT(str, c, false, r);
}

void SplitStringDontTrim(const butil::StringPiece16& str, char16 c,
                         std::vector<butil::StringPiece16>* r) {
    DCHECK(CBU16_IS_SINGLE(c));
    SplitStringT(str, c, false, r);
}
...
void SplitStringDontTrim(const butil::StringPiece& str, char c,
                         std::vector<butil::StringPiece>* r) {
    DCHECK(IsStringUTF8(str));
#if CHAR_MIN < 0
    DCHECK(c >= 0);
