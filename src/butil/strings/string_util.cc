    ...
    for (const wchar_t* position = format; *position != '\0'; ++position) {
        if (*position == '%') {
            ...
            while (in_specification) {
                ...
                if (*++position == '\0') {
                    // The format string ended in the middle of a specification.
                    // Call it portable because no unportable specifications
                    // were found.  The string is equally broken on all
                    // platforms.
                    return true;
                }

                if (*position == 'l') {
                    // 'l' is the only thing that can save the 's' and 'c'
                    // specifiers.
                    modifier_l = true;
                } else if (((*position == 's' || *position == 'c') &&
                            !modifier_l) ||
...
const std::string& EmptyString() { return EmptyStrings::GetInstance()->s; }

const string16& EmptyString16() { return EmptyStrings::GetInstance()->s16; }

template <typename STR>
bool ReplaceCharsT(const STR& input, const STR& replace_chars,
                   const STR& replace_with, STR* output) {
    bool removed          = false;
...
bool ReplaceChars(const string16& input,
                  ...
                  const string16& replace_with, string16* output) {
    return ReplaceCharsT(input, replace_chars.as_string(), replace_with,
                         output);
}
...
bool RemoveChars(const string16& input,
                 const butil::StringPiece16& remove_chars, string16* output) {
    return ReplaceChars(input, remove_chars.as_string(), string16(), output);
}

bool RemoveChars(const std::string& input,
                 const butil::StringPiece& remove_chars, std::string* output) {
...
TrimPositions TrimStringT(const STR& input, const STR& trim_chars,
    ...
    // whitespace from whichever position the caller was interested in.  For
    // empty input, we stripped no whitespace, but we still need to clear
    // |output|.
...
bool TrimString(const string16& input, const butil::StringPiece16& trim_chars,
                string16* output) {
    return TrimStringT(input, trim_chars.as_string(), TRIM_ALL, output) !=
           TRIM_NONE;
}

bool TrimString(const std::string& input, const butil::StringPiece& trim_chars,
                std::string* output) {
    return TrimStringT(input, trim_chars.as_string(), TRIM_ALL, output) !=
           TRIM_NONE;
}
...
TrimPositions TrimWhitespaceASCII(const butil::StringPiece& input,
    ...
    return TrimStringT(input, butil::StringPiece(kWhitespaceASCII), positions,
                       output);
}
...
STR CollapseWhitespaceT(const STR& text, bool trim_sequences_with_line_breaks) {
    ...
    // Set flags to pretend we're already in a trimmed whitespace sequence, so
    // we will trim any leading whitespace.
    ...
    for (typename STR::const_iterator i(text.begin()); i != text.end(); ++i) {
        if (IsWhitespace(*i)) {
            ...
            if (trim_sequences_with_line_breaks && !already_trimmed &&
                ((*i == '\n') || (*i == '\r'))) {
                // Whitespace sequences containing CR or LF are eliminated
                // entirely.
...
bool IsStringASCII(const StringPiece& str) { return DoIsStringASCII(str); }
...
static inline bool DoLowerCaseEqualsASCII(Iter a_begin, Iter a_end,
                                          const char* b) {
    for (Iter it = a_begin; it != a_end; ++it, ++b) {
        if (!*b || butil::ToLowerASCII(*it) != *b) return false;
    }
    return *b == 0;
}
...
bool LowerCaseEqualsASCII(const char16* a_begin, const char16* a_end,
                          const char* b) {
    return DoLowerCaseEqualsASCII(a_begin, a_end, b);
}
...
bool StartsWithASCII(const std::string& str, const std::string& search,
    ...
    if (case_sensitive)
        ...
        return butil::strncasecmp(str.c_str(), search.c_str(),
                                  search.length()) == 0;
}
...
bool StartsWithT(const STR& str, const STR& search, bool case_sensitive) {
    if (case_sensitive) {
        ...
        return std::equal(
            search.begin(), search.end(), str.begin(),
...
bool EndsWithT(const STR& str, const STR& search, bool case_sensitive) {
    ...
    if (search_length > str_length) return false;
    if (case_sensitive)
        return str.compare(str_length - search_length, search_length, search) ==
               0;
    return std::equal(
...
void DoReplaceSubstringsAfterOffset(StringType* str, size_t start_offset,
...
void ReplaceFirstSubstringAfterOffset(string16* str, size_t start_offset,
...
void ReplaceSubstringsAfterOffset(string16* str, size_t start_offset,
                                  const string16& find_this,
                                  const string16& replace_with) {
    DoReplaceSubstringsAfterOffset(str, start_offset, find_this, replace_with,
...
template <typename STR>
static size_t TokenizeT(const STR& str, const STR& delimiters,
...
OutStringType DoReplaceStringPlaceholders(
    ...
    size_t sub_length = 0;
    for (typename std::vector<OutStringType>::const_iterator iter =
             subst.begin();
         iter != subst.end(); ++iter) {
        ...
        if ('$' == *i) {
            if (i + 1 != format_string.end()) {
                ...
                if ('$' == *i) {
                    ...
                    index -= 1;
                    if (offsets) {
                        ReplacementOffset r_offset(
                            index, static_cast<int>(formatted.size()));
                        r_offsets.insert(
                            std::lower_bound(r_offsets.begin(), r_offsets.end(),
    ...
    if (offsets) {
        for (std::vector<ReplacementOffset>::const_iterator i =
                 r_offsets.begin();
             i != r_offsets.end(); ++i) {
...
string16 ReplaceStringPlaceholders(const string16& format_string,
                                   const string16& a, size_t* offset) {
    std::vector<size_t> offsets;
    ...
    while (*pattern != pattern_end && *string != string_end) {
        ...
        if (pattern_char == next(&string_next, string_end) &&
            ...
            *string  = string_next;
            ...
            // escapement, that means that it was an error to advance the ptr
            // here, let's put it back where it was. This also mean that the
            // MatchPattern function will return false because if we can't match
            // an escape char here, then no one will.
            if (escape) {
                *pattern = escape;
            }
            return;
...
static bool MatchPatternT(const CHAR* eval, const CHAR* eval_end,
    ...
    if (pattern[0] == '*') {
        ...
        // We reached the end of the string, let see if the pattern contains
        // only wildcards.
        if (eval == eval_end) {
            EatWildcard(&pattern, pattern_end, next);
            if (pattern != pattern_end) return false;
...
bool MatchPattern(const string16& eval, const string16& pattern) {
    return MatchPatternT(eval.c_str(), eval.c_str() + eval.size(),
                         pattern.c_str(), pattern.c_str() + pattern.size(), 0,
                         NextCharUTF16());
}
