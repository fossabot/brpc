...
public:
    ...
    // The string object must live longer than the tokenizer.  (In particular
    // this should not be constructed with a temporary.)
    StringTokenizerT(const str& string, const str& delims) {
        Init(string.begin(), string.end(), delims);
    }

    StringTokenizerT(const_iterator string_begin, const_iterator string_end,
                     const str& delims) {
    ...
    // it ignores delimiters that it finds.  It switches out of this mode once
    // it finds another instance of the quote char.  If a backslash is
    // encountered within a quoted string, then the next character is skipped.
...
private:
    void Init(const_iterator string_begin, const_iterator string_end,
              const str& delims) {
    ...
    bool QuickGetNext() {
        ...
        for (;;) {
            ...
            if (token_end_ == end_) return false;
            ++token_end_;
            if (delims_.find(*token_begin_) == str::npos) break;
    ...
    bool FullGetNext() {
        ...
        for (;;) {
            ...
            if (token_end_ == end_) return false;
            ++token_end_;
            if (AdvanceOne(&state, *token_begin_)) break;
