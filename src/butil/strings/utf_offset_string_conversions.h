...
public:
    struct BUTIL_EXPORT Adjustment {
        Adjustment(size_t original_offset, size_t original_length,
    ...
    // Adjusts all offsets in |offsets_for_adjustment| to reflect the
    // adjustments recorded in |adjustments|.
    //
    // Offsets represents insertion/selection points between characters: if
    // |src| is "abcd", then 0 is before 'a', 2 is between 'b' and 'c', and 4 is
    // at the end of the string.  Valid input offsets range from 0 to |src_len|.
    // On exit, each offset will have been modified to point at the same logical
    // position in the output string.  If an offset cannot be successfully
    // adjusted (e.g., because it points into the middle of a multibyte
    // sequence), it will be set to string16::npos.
    ...
    // |adjustments|.
    static void AdjustOffset(const Adjustments& adjustments, size_t* offset);

    // Adjusts all offsets in |offsets_for_unadjustment| to reflect the reverse
    // of the adjustments recorded in |adjustments|.  In other words, the
    // offsets provided represent offsets into an adjusted string and the caller
    // wants to know the offsets they correspond to in the original string.  If
    // an offset cannot be successfully unadjusted (e.g., because it points into
...
BUTIL_EXPORT string16
UTF8ToUTF16WithAdjustments(const butil::StringPiece& utf8,
                           butil::OffsetAdjuster::Adjustments* adjustments);
...
BUTIL_EXPORT string16
UTF8ToUTF16AndAdjustOffsets(const butil::StringPiece& utf8,
                            std::vector<size_t>* offsets_for_adjustment);
...
struct LimitOffset {
    ...
    void operator()(size_t& offset) {
        if (offset > limit_) offset = T::npos;
    }
