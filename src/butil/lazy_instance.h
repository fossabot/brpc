...
struct DefaultLazyInstanceTraits {
    ...
    static Type* New(void* instance) {
        DCHECK_EQ(reinterpret_cast<uintptr_t>(instance) & (ALIGNOF(Type) - 1),
                  0u)
            << ": Bad boy, the buffer passed to placement new is not aligned!\n"
               "This may break some stuff like SSE-based optimizations "
               "assuming the "
               "<Type> objects are word aligned.";
        // Use placement new to initialize our instance in our preallocated
        // space. The parenthesis is very important here to force POD type
        // initialization.
...
template <typename Type>
const bool
    DefaultLazyInstanceTraits<Type>::kAllowedToAccessOnNonjoinableThread =
        false;
#endif
...
#ifndef NDEBUG
template <typename Type>
const bool LeakyLazyInstanceTraits<Type>::kAllowedToAccessOnNonjoinableThread =
    true;
#endif
...
public:
    ...
    // be created to register the (empty) destructor with atexit() under MSVC,
    // for example. We handle destruction of the contained Type class explicitly
    // via the OnExit member function, where needed. ~LazyInstance() {}
    ...
    Type* Pointer() {
        ...
        // created. Since a thread sees private_instance_ == 0 or
        // kLazyInstanceStateCreating at most once, the load is taken out of
        // NeedsInstance() as a fast-path. The load has acquire memory ordering
        // as a thread which sees private_instance_ > creating needs to acquire
        // visibility over the associated data (private_buf_). Pairing
        // Release_Store is in CompleteLazyInstance().
        ...
        // and CompleteLazyInstance(...) happens before "return instance()"
        // below. See the corresponding HAPPENS_BEFORE in
...
private:
    ...
    // Calling OnExit while the instance is in use by other threads is a
    // mistake.
