...
ThreadIdNameManager::ThreadIdNameManager()
    : main_process_name_(NULL), main_process_id_(kInvalidThreadId) {
...
void ThreadIdNameManager::SetName(PlatformThreadId id, const char* name) {
    ...
    AutoLock locked(lock_);
    NameToInternedNameMap::iterator iter =
        name_to_interned_name_.find(str_name);
...
const char* ThreadIdNameManager::GetName(PlatformThreadId id) {
    ...
    if (id == main_process_id_) return main_process_name_->c_str();
...
void ThreadIdNameManager::RemoveName(PlatformThreadHandle::Handle handle,
    ...
    if (id_to_handle_iter->second != handle) return;
