...
void SimpleThread::Start() {
    ...
    bool success =
        PlatformThread::Create(options_.stack_size(), this, &thread_);
    DCHECK(success);
...
DelegateSimpleThreadPool::DelegateSimpleThreadPool(
    ...
    : name_prefix_(name_prefix), num_threads_(num_threads), dry_(true, false) {}
...
void DelegateSimpleThreadPool::AddWork(Delegate* delegate, int repeat_count) {
    ...
    for (int i = 0; i < repeat_count; ++i) delegates_.push(delegate);
    // If we were empty, signal that we have work now.
    if (!dry_.IsSignaled()) dry_.Signal();
}
...
void DelegateSimpleThreadPool::Run() {
    ...
    while (true) {
        dry_.Wait();
        {
            ...
            if (delegates_.empty()) dry_.Reset();
        }
