...
void** ConstructTlsVector() {
    ...
    if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
        ...
        // The TLS_KEY_OUT_OF_INDEXES is used to find out whether the key is set
        // or not in NoBarrier_CompareAndSwap, but Posix doesn't have invalid
        // key, we define an almost impossible value be it. If we really get
        // TLS_KEY_OUT_OF_INDEXES as value of key, just alloc another TLS slot.
        if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
        ...
        // TLS_KEY_OUT_OF_INDEXES, go ahead and set it.  Otherwise, do nothing,
        // as another thread already did our dirty work.
        if (PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES !=
            butil::subtle::NoBarrier_CompareAndSwap(
                &g_native_tls_key,
                PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES, key)) {
            // We've been shortcut. Another thread replaced g_native_tls_key
            // first so we need to destroy our index and use the one the other
            // thread got first.
    ...
    // system to initialize, which will include registering for a TLS key.  If
    // we are not careful here, then that request to create a key will call new
    // back, and we'll have an infinite loop.  We avoid that as follows: Use a
    // stack allocated vector, so that we don't have dependence on our allocator
    // until our service is in place.  (i.e., don't even call new until after
    // we're setup)
...
void OnThreadExitInternal(void* value) {
    ...
    // the allocator and cause it to resurrect itself (with no possibly
    // destructor call to follow).  We handle this problem as follows: Switch to
    // using a stack allocated vector, so that we don't have dependence on our
    // allocator after we have called all g_tls_destructors.  (i.e., don't even
    // call delete[] after we're done with destructors.)
    void* stack_allocated_tls_data[kThreadLocalStorageSize];
    ...
    while (need_to_scan_destructors) {
        ...
        // destructor call.  That user was able to function, and define a slot
        // with no other services running, so perhaps it is a basic service
        // (like an allocator) and should also be destroyed last.  If we get the
        // order wrong, then we'll itterate several more times, so it is really
        // not that critical (but it might help).
        ...
        for (int slot = last_used_tls_key; slot > 0; --slot) {
            ...
            destructor(value);
            // Any destructor might have called a different service, which then
            // set a different slot to a non-NULL value.  Hence we need to check
...
void PlatformThreadLocalStorage::OnThreadExit() {
    PlatformThreadLocalStorage::TLSKey key =
        butil::subtle::NoBarrier_Load(&g_native_tls_key);
    if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) return;
...
void* ThreadLocalStorage::StaticSlot::Get() const {
    void** tls_data =
        static_cast<void**>(PlatformThreadLocalStorage::GetTLSValue(
            butil::subtle::NoBarrier_Load(&g_native_tls_key)));
