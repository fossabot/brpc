...
public:
    ...
    // that to support a much larger number of Chromium slots (independent of
    // the OS restrictions). The following returns true if it successfully is
    // able to return an OS key in |key|.
...
public:
    // Prototype for the TLS destructor function, which can be optionally used
    // to cleanup thread local storage on thread exit.  'value' is the data that
    // is stored in thread local storage.
    typedef void (*TLSDestructorFunc)(void* value);

    // StaticSlot uses its own struct initializer-list style static
    // initialization, as base's LINKER_INITIALIZED requires a constructor and
    // on some compilers (notably gcc 4.4) this still ends up needing runtime
    // initialization.
#define TLS_INITIALIZER \
    ...
    struct BUTIL_EXPORT StaticSlot {
        ...
        // 'destructor' is a pointer to a function to perform per-thread cleanup
        // of this object.  If set to NULL, no cleanup is done for this TLS
        // slot. Returns false on error.
