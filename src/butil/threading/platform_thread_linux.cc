...
#if !defined(OS_NACL)
    // On linux we can get the thread names to show up in the debugger by
...
void PlatformThread::SetThreadPriority(PlatformThreadHandle handle,
    ...
    if (priority == kThreadPriority_RealtimeAudio) {
        ...
        if (pthread_setschedparam(pthread_self(), SCHED_RR, &kRealTimePrio) ==
            0) {
    ...
    if (setpriority(PRIO_PROCESS, handle.id_, kNiceSetting)) {
        DVPLOG(1) << "Failed to set nice value of thread (" << handle.id_
                  << ") to " << kNiceSetting;
    }
#endif  //  !defined(OS_NACL)
}
