...
public:
    ...
    PlatformThreadHandle() : handle_(0), id_(0) {}

    explicit PlatformThreadHandle(Handle handle) : handle_(handle), id_(0) {}

    PlatformThreadHandle(Handle handle, PlatformThreadId id)
...
public:
    ...
    // that the Delegate object outlives the thread.
    static bool Create(size_t stack_size, Delegate* delegate,
                       PlatformThreadHandle* thread_handle);

    // CreateWithPriority() does the same thing as Create() except the priority
    // of the thread is set based on |priority|.  Can be used in place of
    // Create() followed by SetThreadPriority().  SetThreadPriority() has not
    // been implemented on the Linux platform yet, this is the only way to get a
    // high priority thread on Linux.
    ...
    // blocks the caller until the designated thread exits.  This will
    // invalidate |thread_handle|.
