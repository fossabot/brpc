...
Watchdog::Watchdog(const TimeDelta& duration,
                   const std::string& thread_watched_name, bool enabled)
    : enabled_(enabled)
    , lock_()
    ...
    , delegate_(this) {
    if (!enabled_) return;  // Don't start thread, or doing anything really.
...
void Watchdog::Arm() { ArmAtStartTime(TimeTicks::Now()); }
...
void Watchdog::ArmAtStartTime(const TimeTicks start_time) {
    ...
    // Force watchdog to wake up, and go to sleep with the timer ticking with
    // the proper duration.
    condition_variable_.Signal();
}
...
void Watchdog::ThreadDelegate::ThreadMain() {
    ...
    while (1) {
        ...
        if (remaining_duration.InMilliseconds() > 0) {
            // Spurios wake?  Timer drifts?  Go back to sleep for remaining
            // time.
        ...
        // Watch out for a user that stopped the debugger on a different alarm!
        {
            ...
            if (static_data->last_debugged_alarm_time >
                watchdog_->start_time_) {
                // False alarm: we started our clock before the debugger break
                // (last alarm time).
                watchdog_->start_time_ +=
                    static_data->last_debugged_alarm_delay;
                if (static_data->last_debugged_alarm_time >
                    watchdog_->start_time_)
        ...
        if (last_alarm_delay <= TimeDelta::FromMilliseconds(2)) continue;
