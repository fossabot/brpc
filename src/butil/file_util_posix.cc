...
bool RealPath(const FilePath& path, FilePath* real_path) {
    ...
    if (!realpath(path.value().c_str(), buf)) return false;
...
bool DetermineDevShmExecutable() {
    ...
    if (fd.is_valid()) {
        ...
        void* mapping =
            mmap(NULL, pagesize, PROT_READ, MAP_SHARED, fd.get(), 0);
...
FilePath MakeAbsoluteFilePath(const FilePath& input) {
    ...
    if (realpath(input.value().c_str(), full_path) == NULL) return FilePath();
...
bool ReplaceFile(const FilePath& from_path, const FilePath& to_path,
    ...
    if (rename(from_path.value().c_str(), to_path.value().c_str()) == 0)
        return true;
    if (error) *error = File::OSErrorToFileError(errno);
    return false;
}
...
bool CopyDirectory(const FilePath& from_path, const FilePath& to_path,
    ...
    while (success && !current.empty()) {
        ...
        if (S_ISDIR(from_stat.st_mode)) {
            if (mkdir(target_path.value().c_str(), from_stat.st_mode & 01777) !=
                    0 &&
                errno != EEXIST) {
        ...
        current = traversal.Next();
        if (!current.empty()) from_stat = traversal.GetInfo().stat();
    }
...
bool ReadSymbolicLink(const FilePath& symlink_path, FilePath* target_path) {
    ...
    char buf[PATH_MAX];
    ssize_t count =
        ::readlink(symlink_path.value().c_str(), buf, arraysize(buf));
...
bool GetPosixFilePermissions(const FilePath& path, int* mode) {
    ...
    if (CallStat(path.value().c_str(), &file_info) != 0) return false;
...
bool SetPosixFilePermissions(const FilePath& path, int mode) {
    ...
    if (CallStat(path.value().c_str(), &stat_buf) != 0) return false;
...
bool CreateDirectoryAndGetError(const FilePath& full_path, File::Error* error,
    ...
    if (!create_parents) {
        ...
        if (mkdir(full_path.value().c_str(),
                  0755 /*others can read the dir*/) == 0) {
            return true;
        }
...
bool NormalizeFilePath(const FilePath& path, FilePath* normalized_path) {
    ...
    if (!RealPath(path, &real_path_result)) return false;
...
bool IsLink(const FilePath& file_path) {
    ...
    if (CallLstat(file_path.value().c_str(), &st) != 0) return false;
...
bool GetFileInfo(const FilePath& file_path, File::Info* results) {
    ...
    if (file_path.IsContentUri()) {
        ...
        if (CallStat(file_path.value().c_str(), &file_info) != 0) return false;
#if defined(OS_ANDROID)
    }
#endif  // defined(OS_ANDROID)
...
bool MoveUnsafe(const FilePath& from_path, const FilePath& to_path) {
    ...
    if (CallStat(to_path.value().c_str(), &to_file_info) == 0) {
        ...
        if (CallStat(from_path.value().c_str(), &from_file_info) == 0) {
            if (S_ISDIR(to_file_info.st_mode) !=
                S_ISDIR(from_file_info.st_mode))
                return false;
        } else {
            return false;
        }
...
bool CopyFileUnsafe(const FilePath& from_path, const FilePath& to_path) {
    ...
    while (result) {
        ssize_t bytes_read =
            HANDLE_EINTR(read(infile, &buffer[0], buffer.size()));
        if (bytes_read < 0) {
            result = false;
            break;
        }
