...
public:
    ...
    void Reset(const butil::Callback<void(void)>& callback) {
        ...
        // Outstanding tasks (e.g., posted to a message loop) must not be
        // called.
        Cancel();
    ...
    const butil::Callback<void(void)>& callback() const { return forwarder_; }
...
public:
    ...
    // Returns a callback that can be disabled by calling Cancel().
    const butil::Callback<void(A1)>& callback() const { return forwarder_; }
...
public:
    ...
    // Returns a callback that can be disabled by calling Cancel().
    const butil::Callback<void(A1, A2)>& callback() const { return forwarder_; }
