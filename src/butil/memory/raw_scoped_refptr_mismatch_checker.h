...
#if defined(OS_WIN)
    enum { value = butil::false_type::value };
#else
    enum {
        // Human readable translation: you needed to be a scoped_refptr if you
        // are a
        // raw pointer type and are convertible to a
        // RefCounted(Base|ThreadSafeBase)
        // type.
