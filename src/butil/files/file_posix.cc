...
COMPILE_ASSERT(File::FROM_BEGIN == SEEK_SET && File::FROM_CURRENT == SEEK_CUR &&
...
static int CallFsync(PlatformFile file) { return HANDLE_EINTR(fsync(file)); }
...
void File::Info::FromStat(const stat_wrapper_t& stat_info) {
    ...
    last_modified = Time::FromTimeT(last_modified_sec) +
                    TimeDelta::FromMicroseconds(
                        last_modified_nsec / Time::kNanosecondsPerMicrosecond);

    last_accessed = Time::FromTimeT(last_accessed_sec) +
                    TimeDelta::FromMicroseconds(
                        last_accessed_nsec / Time::kNanosecondsPerMicrosecond);

    creation_time = Time::FromTimeT(creation_time_sec) +
                    TimeDelta::FromMicroseconds(
                        creation_time_nsec / Time::kNanosecondsPerMicrosecond);
}
...
void File::InitializeUnsafe(const FilePath& name, uint32_t flags) {
    ...
    if (flags & FLAG_OPEN_ALWAYS) {
        if (descriptor < 0) {
            ...
            if (flags & FLAG_EXCLUSIVE_READ || flags & FLAG_EXCLUSIVE_WRITE)
                open_flags |=
                    O_EXCL;  // together with O_CREAT implies O_NOFOLLOW
    ...
    if (flags & FLAG_DELETE_ON_CLOSE) unlink(name.value().c_str());
...
int File::ReadAtCurrentPos(char* data, int size) {
    ...
    do {
        rv = HANDLE_EINTR(
            read(file_.get(), data + bytes_read, size - bytes_read));
        if (rv <= 0) break;
...
int64_t File::GetLength() {
    ...
    if (CallFstat(file_.get(), &file_info)) return false;
...
File::Error File::Lock() { return CallFctnlFlock(file_.get(), true); }

File::Error File::Unlock() { return CallFctnlFlock(file_.get(), false); }
