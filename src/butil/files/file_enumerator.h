...
public:
    ...
    public:
        ...
        // The name of the file. This will not include any path information.
        // This is in constrast to the value returned by FileEnumerator.Next()
        // which includes the |root_path| passed into the FileEnumerator
        // constructor.
    ...
    // |root_path| is the starting directory to search for. It may or may not
    // end in a slash.
    //
    ...
    // since the underlying code uses OS-specific matching routines.  In
    // general, Windows matching is less featureful than others, so test there
    // first. If unspecified, this will match all files. NOTE: the pattern only
    // matches the contents of root_path, not files in recursive subdirectories.
