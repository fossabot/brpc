...
bool EqualDriveLetterCaseInsensitive(const StringType& a, const StringType& b) {
    ...
    if (!StartsWith(a_letter, b_letter, false)) return false;
...
bool AreAllSeparators(const StringType& input) {
    for (StringType::const_iterator it = input.begin(); it != input.end();
         ++it) {
...
StringType::size_type ExtensionSeparatorPosition(const StringType& path) {
    ...
    if (last_dot == StringType::npos || last_dot == 0U) return last_dot;
    ...
    for (size_t i = 0; i < arraysize(kCommonDoubleExtensionSuffixes); ++i) {
        if (LowerCaseEqualsASCII(extension,
...
FilePath::FilePath(const StringType& path) : path_(path) {
    ...
    if (nul_pos != StringType::npos) path_.erase(nul_pos, StringType::npos);
}
...
bool FilePath::AppendRelativePath(const FilePath& child, FilePath* path) const {
    ...
    if ((FindDriveLetter(*parent_comp) != StringType::npos) &&
        ...
        if (!StartsWith(*parent_comp, *child_comp, false)) return false;
        ++parent_comp;
        ++child_comp;
    }
#endif  // defined(FILE_PATH_USES_DRIVE_LETTERS)
...
StringType FilePath::Extension() const {
    ...
    if (dot == StringType::npos) return StringType();
...
StringType FilePath::FinalExtension() const {
    FilePath base(BaseName());
    const StringType::size_type dot =
        FinalExtensionSeparatorPosition(base.path_);
    if (dot == StringType::npos) return StringType();
...
FilePath FilePath::RemoveExtension() const {
    ...
    if (dot == StringType::npos) return *this;

    return FilePath(path_.substr(0, dot));
}

FilePath FilePath::RemoveFinalExtension() const {
    if (FinalExtension().empty()) return *this;
...
FilePath FilePath::ReplaceExtension(const StringType& extension) const {
    ...
    // If the new extension is "" or ".", then just remove the current
    // extension.
    if (extension.empty() || extension == StringType(1, kExtensionSeparator))
        return no_ext;
...
bool FilePath::MatchesExtension(const StringType& extension) const {
    ...
    if (current_extension.length() != extension.length()) return false;
...
FilePath FilePath::Append(const StringType& component) const {
    ...
    if (appended->length() > 0 && new_path.path_.length() > 0) {
        ...
        if (!IsSeparator(new_path.path_[new_path.path_.length() - 1])) {
            ...
            if (FindDriveLetter(new_path.path_) + 1 !=
...
std::string FilePath::MaybeAsASCII() const {
    if (butil::IsStringASCII(path_)) return path_;
    return std::string();
}
...
#elif defined(OS_WIN)
string16 FilePath::LossyDisplayName() const { return path_; }

std::string FilePath::MaybeAsASCII() const {
    if (butil::IsStringASCII(path_)) return UTF16ToASCII(path_);
    return std::string();
}

std::string FilePath::AsUTF8Unsafe() const { return WideToUTF8(value()); }

string16 FilePath::AsUTF16Unsafe() const { return value(); }
...
int FilePath::CompareIgnoreCase(const StringType& string1,
    ...
    if (i1 != string1end) return 1;
    if (i2 != string2end) return -1;
    return 0;
}
...
inline int HFSReadNextNonIgnorableCodepoint(const char* string, int length,
                                            int* index) {
    int codepoint = 0;
    while (*index < length && codepoint == 0) {
        ...
        if (codepoint > 0) {
            ...
            if (lookup_offset != 0)
                codepoint =
                    lower_case_table[lookup_offset + (codepoint & 0x00FF)];
            // Note: codepoint1 may be again 0 at this point if the character
            // was an ignorable.
        }
    }
    return codepoint;
...
int FilePath::HFSFastUnicodeCompare(const StringType& string1,
    ...
    for (;;) {
        int codepoint1 =
            HFSReadNextNonIgnorableCodepoint(string1.c_str(), length1, &index1);
        int codepoint2 =
            HFSReadNextNonIgnorableCodepoint(string2.c_str(), length2, &index2);
        if (codepoint1 != codepoint2) return (codepoint1 < codepoint2) ? -1 : 1;
        if (codepoint1 == 0) {
...
StringType FilePath::GetHFSDecomposedForm(const StringType& string) {
    ScopedCFTypeRef<CFStringRef> cfstring(CFStringCreateWithBytesNoCopy(
        NULL, reinterpret_cast<const UInt8*>(string.c_str()), string.length(),
...
int FilePath::CompareIgnoreCase(const StringType& string1,
    ...
    if (string1.empty()) return string2.empty() ? 0 : -1;
    ...
    if (hfs1.empty() || hfs2.empty()) {
        ...
        ScopedCFTypeRef<CFStringRef> cfstring1(CFStringCreateWithBytesNoCopy(
            NULL, reinterpret_cast<const UInt8*>(string1.c_str()),
            string1.length(), kCFStringEncodingUTF8, false, kCFAllocatorNull));
        ScopedCFTypeRef<CFStringRef> cfstring2(CFStringCreateWithBytesNoCopy(
            NULL, reinterpret_cast<const UInt8*>(string2.c_str()),
            string2.length(), kCFStringEncodingUTF8, false, kCFAllocatorNull));
