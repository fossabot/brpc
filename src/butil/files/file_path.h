...
public:
    ...
    // this is the only situation in which BaseName will return an absolute
    // path.
    ...
    // NOTE: this is slightly different from the similar file_util
    // implementation which returned simply 'jojo'.
    ...
    // Examples:
    // path == "C:\pics\jojo.jpg" suffix == " (1)", returns "C:\pics\jojo
    // (1).jpg" path == "jojo.jpg"         suffix == " (1)", returns "jojo
    // (1).jpg" path == "C:\pics\jojo"     suffix == " (1)", returns
    // "C:\pics\jojo (1)" path == "C:\pics.old\jojo" suffix == " (1)", returns
    // "C:\pics.old\jojo (1)"
    FilePath InsertBeforeExtension(const StringType& suffix) const
        WARN_UNUSED_RESULT;
    FilePath InsertBeforeExtensionASCII(const butil::StringPiece& suffix) const
        WARN_UNUSED_RESULT;
    ...
    // Returns true if the file path matches the specified extension. The test
    ...
    FilePath AppendASCII(const butil::StringPiece& component) const
        WARN_UNUSED_RESULT;
    ...
    // Returns the string in the special canonical decomposed form as defined
    // for HFS, which is close to, but not quite, decomposition form D. See
...
private:
    ...
    // separators is never stripped, to support alternate roots.  This is used
    // to support UNC paths on Windows.
...
BUTIL_EXPORT extern void PrintTo(const butil::FilePath& path,
                                 std::ostream* out);
