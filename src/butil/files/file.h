...
public:
    // FLAG_(OPEN|CREATE).* are mutually exclusive. You should specify exactly
    // one of the five (possibly combining with other flags) when opening or
    // creating a file. FLAG_(WRITE|APPEND) are mutually exclusive. This is so
    ...
    enum Flags {
        ...
        FLAG_WRITE          = 1 << 6,
        FLAG_APPEND         = 1 << 7,
        FLAG_EXCLUSIVE_READ = 1
                              << 8,  // EXCLUSIVE is opposite of Windows SHARE.
    ...
    // fields needs to change, please ensure that those histograms are obsolete
    // or have been moved to a different enum.
    //
    ...
    // file_util_{win|posix}.cc too, and the
    // ParamTraits<butil::PlatformFileInfo> implementation in
    ...
    // the |name| contains path traversal ('..') components.
    ...
    // the given offset. Returns the number of bytes read, or -1 on error. Note
    // that this function makes a best effort to read all data on all platforms,
    // so it is not intended for stream oriented files but instead for cases
    // when the normal expectation is that actually |size| bytes are read unless
    // there is an error.
    ...
    // data on all platforms. Ignores the offset and writes to the end of the
    // file if the file was opened with FLAG_APPEND.
