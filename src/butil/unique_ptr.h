...
// An is_convertible<From, To> that considers From an rvalue (consistent with
// C++0X).
//   This is a simplified version neglecting the types function, array, void and
//   abstract types I had to make a special case out of is_convertible<T,T> to
//   make move-only types happy.
...
struct is_convertible {
    static const bool value =
        sizeof(is_conv_imp::test1<T2>(is_conv_imp::source<T1>())) == 1;
};

template <class T>
struct is_convertible<T, T> {
    static const bool value =
        sizeof(is_conv_imp::test2<T>(is_conv_imp::source<T>())) == 1;
};
...
template <class T>
inline typename butil::enable_if<
    !up_detail::is_convertible<T, up_detail::rv<T> >::value, T&>::type
move(T& t) {
    return t;
}
...
template <class T>
inline typename butil::enable_if<
    !up_detail::is_convertible<T, up_detail::rv<T> >::value, const T&>::type
move(const T& t) {
    return t;
}
...
template <class T>
inline typename butil::enable_if<butil::is_reference<T>::value, T>::type
...
// A move-aware but stripped-down compressed_pair which only optimizes storage
// for T2
...
inline void swap(UniquePtrStorage<T1, T2, b>& x,
                 UniquePtrStorage<T1, T2, b>& y) {
    using std::swap;
...
struct default_delete {
    ...
    default_delete(const default_delete<U>&,
                   typename butil::enable_if<
                       up_detail::is_convertible<U*, T*>::value>::type* = 0) {}
...
static two test(...);
template <class U>
static one test(typename U::pointer* = 0);
...
struct pointer_type {
    typedef typename pointer_type_imp::pointer_type<
        T, typename butil::remove_reference<D>::type>::type type;
};
...
public:
    ...
    typedef typename up_detail::pointer_type<element_type, deleter_type>::type
        pointer;
...
private:
    ...
    typedef typename butil::add_reference<const deleter_type>::type
...
public:
    ...
    unique_ptr(
        pointer p,
        typename butil::conditional<
            butil::is_reference<D>::value,
            volatile typename butil::remove_reference<D>::type&, D>::type d)
        : ptr_(move(p),
               forward<D>(
                   const_cast<typename butil::add_reference<D>::type>(d))) {}

    template <class U, class E>
    unique_ptr(unique_ptr<U, E> u,
               typename butil::enable_if<
                   !butil::is_array<U>::value &&
                   up_detail::is_convertible<typename unique_ptr<U>::pointer,
                                             pointer>::value &&
                   up_detail::is_convertible<E, deleter_type>::value &&
                   (!butil::is_reference<deleter_type>::value ||
                    butil::is_same<deleter_type, E>::value)>::type* = 0)
        : ptr_(u.release(), forward<D>(forward<E>(u.get_deleter()))) {}

    ~unique_ptr() {
        BAIDU_CASSERT(!butil::is_reference<deleter_type>::value,
                      deleter_is_ref);
        BAIDU_CASSERT(!butil::is_pointer<deleter_type>::value,
                      deleter_is_pointer);
        reset();
    }
...
private:
    template <class U>
    explicit unique_ptr(
        U, typename butil::enable_if<
               up_detail::is_convertible<U, pointer>::value>::type* = 0);
...
inline bool operator==(const unique_ptr<T1, D1>& x,
