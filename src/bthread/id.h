...
int bthread_id_create(bthread_id_t* id, void* data,
                      int (*on_error)(bthread_id_t id, void* data,
                                      int error_code));
...
#define bthread_id_lock_and_reset_range(id, pdata, range) \
    bthread_id_lock_and_reset_range_verbose(              \
        id, pdata, range, __FILE__ ":" BAIDU_SYMBOLSTR(__LINE__))
...
void bthread_id_list_swap(bthread_id_list_t* dest, bthread_id_list_t* src);
...
#define bthread_id_error2(id, ec, et)     \
    bthread_id_error2_verbose(id, ec, et, \
