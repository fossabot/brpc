...
public:
    typedef butil::FlatMap<SampledContention*, SampledContention*,
                           ContentionHash, ContentionEqual>
        ContentionMap;
...
void ContentionProfiler::flush_to_disk(bool ending) {
    ...
    if (!_dedup_map.empty()) {
        ...
        for (ContentionMap::const_iterator it = _dedup_map.begin();
    ...
    butil::fd_guard fd(
        open(_filename.c_str(), O_WRONLY | O_CREAT | flag, 0666));
    if (fd < 0) {
...
void SampledContention::destroy() { butil::return_object(this); }

// Remember the conflict hashes for troubleshooting, should be 0 at most of
// time.
static butil::static_atomic<int64_t> g_nconflicthash =
...
BUTIL_FORCE_INLINE bool is_contention_site_valid(
    const bthread_contention_site_t& cs) {
    return cs.sampling_range;
}

BUTIL_FORCE_INLINE void make_contention_site_invalid(
    bthread_contention_site_t* cs) {
    cs->sampling_range = 0;
}
...
//   ../sysdeps/unix/syscall-template.S:81 #1  0x00000000004b4df7 in
//   butil::internal::SpinLockDelay(int volatile*, int, int) () #2
//   0x00000000004b4d57 in SpinLock::SlowLock() () #3  0x00000000004b4a63 in
//   tcmalloc::ThreadCache::InitModule() () #4  0x00000000004aa2b5 in
//   tcmalloc::ThreadCache::GetCache() () #5  0x000000000040c6c5 in (anonymous
//   namespace)::do_malloc_no_errno(unsigned long) [clone.part.16] () #6
//   0x00000000006fc125 in tc_calloc () #7  0x00007effdd245690 in _dlerror_run
//   (operate=operate@entry=0x7effdd245130 <dlsym_doit>,
//   args=args@entry=0x7fff483dedf0) at dlerror.c:141 #8  0x00007effdd245198 in
//   __dlsym (handle=<optimized out>, name=<optimized out>) at dlsym.c:70 #9
//   0x0000000000666517 in bthread::init_sys_mutex_lock () at
//   bthread/mutex.cpp:358 #10 0x00007effddc97a90 in pthread_once () at
//   ../nptl/sysdeps/unix/sysv/linux/x86_64/pthread_once.S:103 #11
//   0x000000000066649f in bthread::first_sys_pthread_mutex_lock (mutex=0xbaf880
//   <_ULx86_64_lock>) at bthread/mutex.cpp:366 #12 0x00000000006678bc in
//   pthread_mutex_lock_impl (mutex=0xbaf880 <_ULx86_64_lock>) at
//   bthread/mutex.cpp:489 #13 pthread_mutex_lock
//   (__mutex=__mutex@entry=0xbaf880 <_ULx86_64_lock>) at bthread/mutex.cpp:751
...
//   uc=0x7fff483def90) at x86_64/Ginit_local.c:47 #16 0x00000000004b5012 in
//   GetStackTrace(void**, int, int) () #17 0x00000000004b2095 in
//   tcmalloc::PageHeap::GrowHeap(unsigned long) () #18 0x00000000004b23a3 in
//   tcmalloc::PageHeap::New(unsigned long) () #19 0x00000000004ad457 in
//   tcmalloc::CentralFreeList::Populate() () #20 0x00000000004ad628 in
//   tcmalloc::CentralFreeList::FetchFromSpansSafe() () #21 0x00000000004ad6a3
//   in tcmalloc::CentralFreeList::RemoveRange(void**, void**, int) () #22
//   0x00000000004b3ed3 in tcmalloc::ThreadCache::FetchFromCentralCache(unsigned
//   long, unsigned long) () #23 0x00000000006fbb9a in tc_malloc ()
...
inline bthread_contention_site_t* add_pthread_contention_site(
    pthread_mutex_t* mutex) {
    MutexMapEntry& entry =
        g_mutex_map[hash_mutex_ptr(mutex) & (MUTEX_MAP_SIZE - 1)];
    ...
    if (expected == 0 || (expected >> PTR_BITS) !=
                             (g_cp_version & ((1 << (64 - PTR_BITS)) - 1))) {
        uint64_t desired = (g_cp_version << PTR_BITS) | (uint64_t)mutex;
        if (m.compare_exchange_strong(expected, desired,
...
inline bool remove_pthread_contention_site(
    ...
    MutexMapEntry& entry =
        g_mutex_map[hash_mutex_ptr(mutex) & (MUTEX_MAP_SIZE - 1)];
    butil::static_atomic<uint64_t>& m = entry.versioned_mutex;
    if ((m.load(butil::memory_order_relaxed) &
         ((((uint64_t)1) << PTR_BITS) - 1)) != (uint64_t)mutex) {
