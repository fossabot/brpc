...
protected:
    TaskIteratorBase(TaskNode* head, ExecutionQueueBase* queue, bool is_stopped,
                     bool high_priority)
        : _cur_node(head)
        , _head(head)
        , _q(queue)
...
struct TaskOptions {
    ...
    // NOTE: We don't guarantee any kind of real-time as there might be tasks
    // still in process which are uninterruptible.
    //
    // Default: false
    bool high_priority;
...
struct ExecutionQueueOptions {
    ...
    // Note that TaskOptions.in_place_if_possible = false will not work, if
    // implementation of Executor is in-place(synchronous).
    Executor* executor;
};
...
template <typename T>
int execution_queue_start(ExecutionQueueId<T>* id,
...
// reference. If you are not sure about the ownership of the return value (which
// releasees the reference of the very ExecutionQueue in the destructor) and
// don't that care the overhead of ExecutionQueue, DON'T use this function
template <typename T>
