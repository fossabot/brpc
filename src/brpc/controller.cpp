...
BAIDU_REGISTER_ERRNO(brpc::ERTMPPUBLISHABLE,
                     "RtmpRetryingClientStream is publishable");
BAIDU_REGISTER_ERRNO(brpc::ERTMPCREATESTREAM,
...
BAIDU_REGISTER_ERRNO(brpc::EH2RUNOUTSTREAMS,
                     "The H2 socket was run out of streams");
...
static bvar::PassiveStatus<std::string> s_rpc_revision("rpc_revision",
                                                       PrintRevision, NULL);
...
Controller::Call::~Call() { CHECK(sending_sock.get() == NULL); }
...
void Controller::set_backup_request_ms(int64_t timeout_ms) {
    if (timeout_ms <= 0x7fffffff) {
        ...
        LOG(WARNING)
            << "backup_request_ms is limited to 0x7fffffff (roughly 24 days)";
    }
}

void Controller::set_max_retry(int max_retry) {
    if (max_retry > MAX_RETRY_COUNT) {
        LOG(WARNING) << "Retry count can't be larger than " << MAX_RETRY_COUNT
                     << ", round it to " << MAX_RETRY_COUNT;
...
bool Controller::Failed() const { return FailedInline(); }

std::string Controller::ErrorText() const { return _error_text; }

void StartCancel(CallId id) { bthread_id_error(id, ECANCELED); }
...
void Controller::AppendServerIdentiy() {
    ...
    if (is_security_mode()) {
        ...
        char ipbuf[64];
        int len = snprintf(ipbuf, sizeof(ipbuf), "%s:%d", butil::my_ip_cstr(),
        ...
        butil::string_appendf(&_error_text, "[%s:%d]", butil::my_ip_cstr(),
                              _server->listen_address().port);
    }
}
...
int Controller::RunOnCancel(bthread_id_t id, void* data, int error_code) {
    ...
    bthread_t th;
    CHECK_EQ(0,
             bthread_start_urgent(&th, NULL, RunOnCancelThread::RunThis, arg));
    return 0;
}
...
void Join(CallId id) { bthread_id_join(id); }

void JoinResponse(CallId id) { bthread_id_join(id); }
    ...
    if (_error_code == EBACKUPREQUEST) {
        ...
        if (timeout_ms() >= 0) {
            rc = bthread_timer_add(
                &_timeout_id, butil::microseconds_to_timespec(_deadline_us),
...
END_OF_RPC:
    if (new_bthread) {
        ...
        bthread_t bt;
        bthread_attr_t attr  = (FLAGS_usercode_in_pthread ? BTHREAD_ATTR_PTHREAD
                                                         : BTHREAD_ATTR_NORMAL);
...
inline bool does_error_affect_main_socket(int error_code) {
    ...
    return error_code == ECONNREFUSED || error_code == ENETUNREACH ||
...
void Controller::Call::OnComplete(Controller* c, int error_code /*note*/,
                                  bool responded, bool end_of_rpc) {
    if (stream_user_data) {
        stream_user_data->DestroyStreamUserData(sending_sock, c, error_code,
                                                end_of_rpc);
        stream_user_data = NULL;
    }
    ...
    if (sending_sock != NULL) {
        ...
        if (enable_circuit_breaker) {
            sending_sock->FeedbackCircuitBreaker(
    ...
    if (need_feedback) {
        const LoadBalancer::CallInfo info = {begin_time_us, peer_id, error_code,
                                             c};
        c->_lb->Feedback(info);
    }
...
void Controller::EndRPC(const CompletionInfo& info) {
    ...
    if (info.id == current_id() || info.id == _correlation_id) {
        ...
        if (_unfinished_call == NULL) {
            CHECK(false) << "A previous non-backup request responded, cid="
                         ...
                         << " stream_user_data="
                         << _current_call.stream_user_data << " sending_sock="
    ...
    if (_done) {
        if (!FLAGS_usercode_in_pthread || _done == DoNothing() /*Note*/) {
            ...
            _done->Run();
            // NOTE: Don't touch this Controller anymore, because it's likely to
            // be deleted by done.
...
void Controller::HandleSendFailed() {
    ...
    const bool new_bthread =
        (_done != NULL && !is_done_allowed_to_run_in_place());
...
void Controller::IssueRPC(int64_t start_realtime_us) {
    ...
    if (SingleServer()) {
        ...
        LoadBalancer::SelectIn sel_in = {start_realtime_us, true,
                                         has_request_code(), _request_code,
                                         _accessed};
    ...
    if (span) {
        if (_current_call.nretry == 0) {
            ...
            span->Annotate("Retrying %s", endpoint2str(_remote_side).c_str());
        }
    }
    ...
    if (_connection_type == CONNECTION_TYPE_SINGLE ||
        ...
        if (_connection_type == CONNECTION_TYPE_POOLED) {
            ...
            tmp_sock.reset();
            SetFailed(EINVAL, "Invalid connection_type=%d",
    ...
    _pack_request(&packet, &user_packet, cid.value, _method, this, _request_buf,
                  using_auth);
    ...
    if (_connect_timeout_ms > 0) {
        if (_deadline_us >= 0) {
            connect_abstime = butil::microseconds_to_timespec(std::min(
                _connect_timeout_ms * 1000L + start_realtime_us, _deadline_us));
        } else {
    ...
    if (span) {
        if (_current_call.nretry == 0) {
            ...
            span->Annotate("Requested(%lld) [%d]", (long long)packet_size,
                           _current_call.nretry + 1);
        }
    }
    if (using_auth) {
...
int Controller::HandleSocketFailed(bthread_id_t id, void* data, int error_code,
    ...
    if (!cntl->is_used_by_rpc()) {
        ...
        cntl->SetFailed(error_code,
                        "Cancel call_id=%" PRId64 " before CallMethod()",
                        id.value);
...
void Controller::HandleStreamConnection(Socket* host_socket) {
    ...
    if (!FailedInline()) {
        if (Socket::Address(_request_stream, &ptr) != 0) {
            if (!FailedInline()) {
                SetFailed(EREQUEST,
                          "Request stream=%" PRIu64
                          " was closed before responded",
                          _request_stream);
            }
...
Controller::CreateProgressiveAttachment(StopStyle stop_style) {
    ...
    _wpa.reset(
        new ProgressiveAttachment(httpsock, http_request().before_http_1_1()));
    return _wpa;
}
...
void Controller::ReadProgressiveAttachmentBy(ProgressiveReader* r) {
    ...
    if (!is_response_read_progressively()) {
        return r->OnEndOfMessage(
            butil::Status(EINVAL,
                          "Can't read progressive attachment from a "
    ...
    if (has_progressive_reader()) {
        return r->OnEndOfMessage(butil::Status(
            EPERM, "%s can't be called more than once", __FUNCTION__));
    }
...
int Controller::GetSockOption(int level, int optname, void* optval,
                              socklen_t* optlen) {
    Socket* s = _current_call.sending_sock.get();
    if (s) {
...
static void RegisterQuitSignalOrDie() {
    ...
    if (prev != SIG_DFL && prev != SIG_IGN) {  // shell may install SIGINT of
...
std::ostream& operator<<(std::ostream& os,
                         const Controller::LogPrefixDummy& p) {
