...
void IndexService::default_method(::google::protobuf::RpcController* controller,
                                  const IndexRequest*, IndexResponse*,
    ...
    if (!as_more) {
        os << Path("/status", html_addr) << " : Status of services" << NL
           ...
           << Path("/flags", html_addr) << " : List all gflags" << NL << SP
           << Path("/flags/port", html_addr) << " : List the gflag" << NL << SP
           << Path("/flags/guard_page_size;help*", html_addr)
           << " : List multiple gflags with glob patterns"
              " (Use $ instead of ? to match single character)"
           << NL << SP
           << "/flags/NAME?setvalue=VALUE : Change a gflag, validator will be "
              "called."
              " User is responsible for thread-safety and consistency issues."
           << NL

           << Path("/vars", html_addr) << " : List all exposed bvars" << NL
           << SP << Path("/vars/rpc_num_sockets", html_addr)
           << " : List the bvar" << NL << SP
           << Path("/vars/rpc_server*_count;iobuf_blo$k_*", html_addr)
           << " : List multiple bvars with glob patterns"
              " (Use $ instead of ? to match single character)"
           << NL

           << Path("/rpcz", html_addr) << " : Recent RPC calls"
           << (!FLAGS_enable_rpcz ? "(disabled)" : "") << NL << SP
           << Path("/rpcz/stats", html_addr) << " : Statistics of rpcz" << NL;
        ...
        os << SP << Path(tmp_oss.str().c_str(), html_addr)
           ...
           << "Other filters: " << MIN_LATENCY_STR << ", "
           << MIN_REQUEST_SIZE_STR << ", " << MIN_RESPONSE_SIZE_STR << ", "
           << LOG_ID_STR << ", " << ERROR_CODE_STR << NL << SP << "/rpcz?"
           << TRACE_ID_STR << "=N : Recent RPC calls whose trace_id is N" << NL
    ...
    os << "curl -H 'Content-Type: application/json' -d 'JSON' " << my_addr
       ...
       << Path("/protobufs", html_addr)
       << " : List all protobuf services and messages" << NL
