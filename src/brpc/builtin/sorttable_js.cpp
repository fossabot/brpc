...
const char* sorttable_js() {
    return "var stIsIE = /*@cc_on!@*/false;\n"
           ...
           "      // table doesn't have a tHead. Since it should have, create "
           "one and\n"
           ...
           "\n"
           "    if (table.tHead.rows.length != 1) return; // can't cope with "
           "two header rows\n"
           "\n"
           "    // Sorttable v1 put rows with a class of \"sortbottom\" at the "
           "bottom (as\n"
           "    // \"total\" rows, for example). This is B&R, since what "
           "you're supposed\n"
           "    // to do is put them in a tfoot. So, if there are sortbottom "
           "rows,\n"
           "    // for backwards compatibility, move them to tfoot (creating "
           "it if needed).\n"
           "    sortbottomrows = [];\n"
           "    for (var i=0; i<table.rows.length; i++) {\n"
           "      if (table.rows[i].className.search(/\\bsortbottom\\b/) != "
           "-1) {\n"
           ...
           "      // manually override the type with a sorttable_type "
           "attribute\n"
           "      if (!headrow[i].className.match(/\\bsorttable_nosort\\b/)) { "
           "// skip this col\n"
           "        mtch = "
           "headrow[i].className.match(/\\bsorttable_([a-z0-9]+)\\b/);\n"
           ...
           "\n"
           "          if (this.className.search(/\\bsorttable_sorted\\b/) != "
           "-1) {\n"
           "            // if we're already sorted by this column, just\n"
           "            // reverse the table, which is quicker\n"
           "            sorttable.reverse(this.sorttable_tbody);\n"
           "            this.className = "
           "this.className.replace('sorttable_sorted',\n"
           "                                                    "
           ...
           "            // if we're already sorted by this column in reverse, "
           "just\n"
           "            // re-reverse the table, which is quicker\n"
           "            sorttable.reverse(this.sorttable_tbody);\n"
           "            this.className = "
           "this.className.replace('sorttable_sorted_reverse',\n"
           "                                                    "
           "'sorttable_sorted');\n"
           "            "
           ...
           "              cell.className = "
           "cell.className.replace('sorttable_sorted','');\n"
           "            }\n"
           "          });\n"
           "          sortfwdind = "
           ...
           "key,\n"
           "            // sort based on the sort keys, and then put the rows "
           "back in order\n"
           ...
           "    // guess the type of a column based on its first non-blank "
           "row\n"
           "    sortfn = sorttable.sort_alpha;\n"
           "    for (var i=0; i<table.tBodies[0].rows.length; i++) {\n"
           "      text = "
           "sorttable.getInnerText(table.tBodies[0].rows[i].cells[column]);\n"
           "      if (text != '') {\n"
           "        if (text.match(/^-?[$]?[\\d,.]+%?$/)) {\n"
           ...
           "          } else {\n"
           "            // looks like a date, but we can't tell which, so "
           "assume\n"
           ...
           "    // this is *not* a generic getInnerText function; it's special "
           "to sorttable.\n"
           ...
           "            innerText += "
           "sorttable.getInnerText(node.childNodes[i]);\n"
           "          }\n"
           ...
           "    // A stable sort function to allow multi-level sorting of "
           "data\n"
           ...
           "                var q = list[i]; list[i] = list[i+1]; list[i+1] = "
           "q;\n"
           "                swap = true;\n"
           "            }\n"
           "        } // for\n"
           ...
           "    event = event || fixEvent(((this.ownerDocument || "
           "this.document || this).parentWindow ||\n"
           "window).event);\n"
           ...
           "            // the object implements a custom forEach method so "
           "use that\n"
