...
void PutVarsHeading(std::ostream& os, bool expand_all) {
    os << "<script language=\"javascript\" type=\"text/javascript\" "
       ...
       << "<style type=\"text/css\">\n"
          ...
          "      $(\"#\" + var_name).bind(\"plothover\", function(event, pos, "
          "item) {\n"
          "        if (item) {\n"
          ...
          "    }\n"
          "  });\n"
       << (expand_all ? "  $(\".variable\").click();\n" :
                      // Set id to "default_expand" to make the graph expand by
                      // default. E.g. latency and qps in /status page are
                      // expanded by default.
       ...
       << "}\n"
          ...
          "  grid: {hoverable:true},\n"
          "  xaxis: { \n"
          "    \"ticks\": [[29,\"-1 day\"],[53,\"-1 hour\"],[113,\"-1 "
          "minute\"]]\n"
          "  }\n"
          "}\n"
          "var cdfOptions = {\n"
          ...
          "        lastPlot[var_name] = $.plot(\"#\" + var_name, "
          "[series.data], trendOptions);\n"
          "        $(\"#value-\" + "
          "var_name).html(series.data[series.data.length - 1][1]);\n"
          "      } else if (series.label == 'cdf') {\n"
          "        lastPlot[var_name] = $.plot(\"#\" + var_name, "
          "[series.data], cdfOptions);\n"
          "        $(\"#value-\" + "
          "var_name).html(series.data[series.data.length - 1][1]);\n"
          "      } else {\n"
          "        lastPlot[var_name] = $.plot(\"#\" + var_name, series, "
          "trendOptions);\n"
       << (bvar::FLAGS_quote_vector ? "        var newValue = '\"[';\n"
                                    : "        var newValue = '[';\n")
       << "        var i;\n"
          ...
          "        }\n"
       << (bvar::FLAGS_quote_vector ? "        newValue += ']\"';\n"
                                    : "        newValue += ']';\n")
       << "        $(\"#value-\" + var_name).html(newValue);\n"
          ...
          "  });\n"
          "  if (enabled[var_name]) {\n"
          "    timeoutId[var_name] = setTimeout(function(){ "
          "fetchData(var_name); }, 1000);\n"
          "  }\n"
          "}\n"
          "$(prepareGraphs);\n"
          "</script>\n";
...
public:
    ...
    void move_to(butil::IOBuf& buf) { _os.move_to(buf); }
...
void VarsService::default_method(::google::protobuf::RpcController* cntl_base,
    ...
    if (with_tabs) {
        ...
        os << "<script type=\"text/javascript\">\n"
              ...
              "    console.log('text changed during searching, search "
              "again');\n"
              "  } else {\n"
              "    searching = false;\n"
              "  }\n"
