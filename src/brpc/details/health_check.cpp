...
DEFINE_int32(health_check_timeout_ms, 500,
...
void OnAppHealthCheckDone::Run() {
    ...
    if (!cntl.Failed() || ptr->Failed()) {
        LOG_IF(INFO, !cntl.Failed())
            << "Succeeded to call " << ptr->remote_side()
            << FLAGS_health_check_path;
...
HealthCheckTask::HealthCheckTask(SocketId id) : _id(id), _first_time(true) {}
...
void HealthCheckTask::OnDestroyingTask() { delete this; }
