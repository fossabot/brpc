...
static int cast_int(void* arg) { return *(int*)arg; }
...
template <typename T>
void OutputTextValue(std::ostream& os, const char* prefix, const T& value) {
    os << prefix << value << "\n";
}

template <typename T>
void OutputValue(std::ostream& os, const char* prefix,
                 const std::string& bvar_name, const T& value,
                 const DescribeOptions& options, bool expand) {
    if (options.use_html) {
        os << "<p class=\"variable";
        if (expand) {
            os << " default_expand";
        ...
        os << "\">" << prefix << "<span id=\"value-" << bvar_name << "\">"
           << value << "</span></p><div class=\"detail\"><div id=\""
           << bvar_name << "\" class=\"flot-placeholder\"></div></div>\n";
    } else {
        return OutputTextValue(os, prefix, value);
    }
}

void MethodStatus::Describe(std::ostream& os,
    ...
    OutputValue(os, "error: ", _nerror_bvar.name(), _nerror_bvar.get_value(),
                options, false);
    OutputValue(os, "eps: ", _eps_bvar.name(), _eps_bvar.get_value(1), options,
                false);
    ...
    if (options.use_html) {
        ...
        OutputTextValue(os,
                        "latency_50: ", _latency_rec.latency_percentile(0.5));
        OutputTextValue(os,
                        "latency_90: ", _latency_rec.latency_percentile(0.9));
        OutputTextValue(os,
                        "latency_99: ", _latency_rec.latency_percentile(0.99));
        OutputTextValue(
            os, "latency_999: ", _latency_rec.latency_percentile(0.999));
        OutputTextValue(
            os, "latency_9999: ", _latency_rec.latency_percentile(0.9999));
    }
    OutputValue(os, "max_latency: ", _latency_rec.max_latency_name(),
                _latency_rec.max_latency(), options, false);

    // Concurrency
    OutputValue(os, "concurrency: ", _nconcurrency_bvar.name(), _nconcurrency,
                options, false);
    if (_cl) {
...
ConcurrencyRemover::~ConcurrencyRemover() {
    if (_status) {
        _status->OnResponded(_c->ErrorCode(),
                             butil::cpuwide_time_us() - _received_us);
        _status = NULL;
    }
