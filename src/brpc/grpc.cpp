...
int64_t ConvertGrpcTimeoutToUS(const std::string* grpc_timeout) {
    ...
    // Only the format that the digit number is equal to (timeout header size -
    // 1) is valid. Otherwise the format is not valid and is treated as no
    // deadline. For example:
    //      "1H", "2993S", "82m" is valid.
    //      "30A" is also valid, but the following switch would fall into
    //      default
    //          case and return -1 since 'A' is not a valid time unit.
    //      "123ASH" is not vaid since the digit number is 3, while the size
    //      is 6. "HHH" is not valid since the dight number is 0, while the size
    //      is 3.
