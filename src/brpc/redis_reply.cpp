...
ParseError RedisReply::ConsumePartialIOBuf(butil::IOBuf& buf) {
    ...
    // Notice that all branches returning PARSE_ERROR_NOT_ENOUGH_DATA must not
    // change `buf'.
    ...
    case ':': {  // Integer       ":<integer>\r\n"
        ...
        intbuf[ncopied]      = '\0';
        const size_t crlf_pos =
            butil::StringPiece(intbuf, ncopied).find("\r\n");
...
public:
    RedisStringPrinter(const char* str, size_t length) : _str(str, length) {}
...
static std::ostream& operator<<(std::ostream& os,
                                const RedisStringPrinter& printer) {
    printer.Print(os);
    return os;
}
...
void RedisStringPrinter::Print(std::ostream& os) const {
    ...
    for (size_t i = 0; i < _str.size(); ++i) {
        ...
        if (c <= 0) {  // unprintable chars
            if (i != flush_start) {
                os << butil::StringPiece(_str.data() + flush_start,
                                         i - flush_start);
            }
            char buf[8] = "\\u0000";
...
void RedisReply::CopyFromDifferentArena(const RedisReply& other) {
    ...
    case REDIS_REPLY_ARRAY: {
        RedisReply* subs =
            (RedisReply*)_arena->allocate(sizeof(RedisReply) * _length);
        if (subs == NULL) {
...
void RedisReply::SetStringImpl(const butil::StringPiece& str,
                               RedisReplyType type) {
    if (_type != REDIS_REPLY_NIL) {
        Reset();
    }
...
void RedisReply::FormatStringImpl(const char* fmt, va_list args,
                                  RedisReplyType type) {
    va_list copied_args;
    ...
    va_end(copied_args);
    if (ret < 0) {
        LOG(FATAL) << "Fail to vsnprintf into buf=" << (void*)buf
                   << " size=" << sizeof(buf);
        return;
