...
struct SendNsheadPbResponse : public google::protobuf::Closure {
    SendNsheadPbResponse(const NsheadPbServiceAdaptor* adaptor,
                         Controller* cntl, NsheadMessage* ns_res,
                         NsheadClosure* done);
...
SendNsheadPbResponse::SendNsheadPbResponse(
    const NsheadPbServiceAdaptor* adaptor2, Controller* cntl2,
    NsheadMessage* ns_res2, NsheadClosure* done2)
    : adaptor(adaptor2)
    , cntl(cntl2)
    , ns_res(ns_res2)
    , done(done2)
...
void SendNsheadPbResponse::Run() {
    ...
    // back response.
    if (saved_status) {
        saved_status->OnResponded(!saved_failed,
                                  butil::cpuwide_time_us() - received_us);
    }
    saved_done->Run();
}

void NsheadPbServiceAdaptor::ProcessNsheadRequest(const Server& server,
