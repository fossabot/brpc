...
struct Protocol {
    ...
    typedef void (*PackRequest)(
        butil::IOBuf* iobuf_out, SocketMessage** user_message_out,
    ...
    // [Optional]
    // Convert `server_addr_and_port'(a parameter to Channel) to
    // butil::EndPoint.
    ...
    typedef const std::string& (*GetMethodName)(
        const google::protobuf::MethodDescriptor* method, const Controller*);
...
const ConnectionType CONNECTION_TYPE_POOLED_AND_SHORT =
    (ConnectionType)((int)CONNECTION_TYPE_POOLED | (int)CONNECTION_TYPE_SHORT);

const ConnectionType CONNECTION_TYPE_ALL =
    (ConnectionType)((int)CONNECTION_TYPE_SINGLE | (int)CONNECTION_TYPE_POOLED |
