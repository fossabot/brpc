...
public:
    RtmpUnsentMessage() : chunk_stream_id(0), new_chunk_size(0), next(NULL) {}
    // @SocketMessage
    butil::Status AppendAndDestroySelf(butil::IOBuf* out, Socket*);
};

// Notice that we can't directly pack CreateStream command in PackRtmpRequest,
// because we need to pack an AMFObject according to
// ctx->can_stream_be_created_with_play_or_publish(), which is in the response
// of Connect command(sent in RtmpConnect::StartConnect).
...
public:
    ...
    virtual void Run(bool error, const RtmpMessageHeader& mh, AMFInputStream*,
                     Socket* socket) = 0;
...
public:
    ...
    void set_create_stream_with_play_or_publish(
        bool create_stream_with_play_or_publish) {
        _create_stream_with_play_or_publish =
    ...
    int SendConnectRequest(const butil::EndPoint& remote_side, int fd,
...
class RtmpChunkStream {
public:
    typedef bool (RtmpChunkStream::*MessageHandler)(const RtmpMessageHeader& mh,
                                                    butil::IOBuf* msg_body,
                                                    Socket* socket);

    typedef bool (RtmpChunkStream::*CommandHandler)(const RtmpMessageHeader& mh,
                                                    AMFInputStream*,
                                                    Socket* socket);
...
public:
    ...
    ParseResult Feed(const RtmpBasicHeader& bh, butil::IOBuf* source,
                     Socket* socket);
    ...
    bool OnMessage(const RtmpBasicHeader& bh, const RtmpMessageHeader& mh,
                   butil::IOBuf* msg_body, Socket* socket);

    bool OnSetChunkSize(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                        Socket* socket);
    bool OnAbortMessage(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                        Socket* socket);
    bool OnAck(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
               Socket* socket);
    ...
    bool OnWindowAckSize(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                         Socket* socket);
    bool OnSetPeerBandwidth(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                            Socket* socket);

    bool OnAudioMessage(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                        Socket* socket);
    bool OnVideoMessage(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                        Socket* socket);
    bool OnDataMessageAMF0(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                           Socket* socket);
    bool OnDataMessageAMF3(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                           Socket* socket);
    ...
    bool OnCommandMessageAMF3(const RtmpMessageHeader& mh,
                              butil::IOBuf* msg_body, Socket* socket);
    bool OnAggregateMessage(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                            Socket* socket);
...
void PackRtmpRequest(butil::IOBuf* buf, SocketMessage**,
