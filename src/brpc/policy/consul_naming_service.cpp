...
DEFINE_string(consul_service_discovery_url, "/v1/health/service/",
...
std::string RapidjsonValueToString(
...
int ConsulNamingService::DegradeToOtherServiceIfNeeded(
    const char* service_name, std::vector<ServerNode>* servers) {
    if (FLAGS_consul_enable_degrade_to_file_naming_service &&
        !_backup_file_loaded) {
        _backup_file_loaded = true;
        const std::string file(FLAGS_consul_file_naming_service_dir +
                               service_name);
...
int ConsulNamingService::GetServers(const char* service_name,
    ...
    if (!_consul_connected) {
        ...
        opt.timeout_ms         = (FLAGS_consul_blocking_query_wait_secs + 10) *
                         butil::Time::kMillisecondsPerSecond;
    ...
    if (!_consul_index.empty()) {
        butil::string_appendf(&consul_url, "&index=%s&wait=%ds",
                              _consul_index.c_str(),
    ...
    if (index != nullptr) {
        if (*index == _consul_index) {
            LOG_EVERY_N(INFO, 100)
                ...
                << service_name << ", consul_index: " << _consul_index;
            return -1;
        }
    } else {
    ...
    if (!services.IsArray()) {
        LOG(ERROR) << "The consul's response for " << service_name
                   << " is not a json array";
        return -1;
    }
    ...
    for (BUTIL_RAPIDJSON_NAMESPACE::SizeType i = 0; i < services.Size(); ++i) {
        ...
        if (itr_address == service.MemberEnd() ||
            !itr_address->value.IsString() || itr_port == service.MemberEnd() ||
        ...
        if (str2endpoint(service["Address"].GetString(),
                         service["Port"].GetUint(), &end_point) != 0) {
        ...
        if (itr_tags != service.MemberEnd()) {
            if (itr_tags->value.IsArray()) {
                if (itr_tags->value.Size() > 0) {
                    // Tags in consul is an array, here we only use the first
                    // one.
                    const BUTIL_RAPIDJSON_NAMESPACE::Value& tag =
                        itr_tags->value[0];
                    if (tag.IsString()) {
                        node.tag = tag.GetString();
                    } else {
                        LOG(ERROR) << "First tag returned by consul is not "
                                      "string, service: "
                                   << RapidjsonValueToString(service);
                        continue;
                    }
                ...
                LOG(ERROR) << "Service tags returned by consul is not json "
                              "array, service: "
                           << RapidjsonValueToString(service);
                continue;
            }
...
int ConsulNamingService::RunNamingService(const char* service_name,
    ...
    for (;;) {
        ...
        if (rc == 0) {
            ...
            if (bthread_usleep(std::max(FLAGS_consul_retry_interval_ms, 1) *
                               butil::Time::kMicrosecondsPerMillisecond) < 0) {
                if (errno == ESTOP) {
                    RPC_VLOG << "Quit NamingServiceThread=" << bthread_self();
                    return 0;
                }
...
void ConsulNamingService::Destroy() { delete this; }
