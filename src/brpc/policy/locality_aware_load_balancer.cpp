...
DEFINE_double(punish_inflight_ratio, 1.5,
              "Decrease weight proportionally if "
...
// 1008680231
static const int64_t WEIGHT_SCALE = std::numeric_limits<int64_t>::max() /
                                    72000000 / (INITIAL_WEIGHT_TREE_SIZE - 1);

LocalityAwareLoadBalancer::LocalityAwareLoadBalancer() : _total(0) {}

LocalityAwareLoadBalancer::~LocalityAwareLoadBalancer() {
    _db_servers.ModifyWithForeground(RemoveAll);
}

bool LocalityAwareLoadBalancer::Add(Servers& bg, const Servers& fg, SocketId id,
...
bool LocalityAwareLoadBalancer::Remove(Servers& bg, SocketId id,
                                       LocalityAwareLoadBalancer* lb) {
...
size_t LocalityAwareLoadBalancer::BatchAdd(Servers& bg, const Servers& fg,
...
int LocalityAwareLoadBalancer::SelectServer(const SelectIn& in,
                                            SelectOut* out) {
        ...
        if (dice < left) {
            ...
            if ((ntry + 1) == n  // Instead of fail with EHOSTDOWN, we prefer
                ...
                if (r.weight_diff) {
                    ...
                    _total.fetch_add(r.weight_diff,
            ...
            const int64_t diff = info.weight->MarkFailed(index, total / n);
            if (diff) {
...
int64_t LocalityAwareLoadBalancer::Weight::Update(const CallInfo& ci,
    ...
    if (ci.error_code == 0) {
        ...
        const int64_t err_latency =
            (nleft * (int64_t)(latency * FLAGS_punish_error_ratio) +
             ndone * ci.controller->timeout_ms() * 1000L) /
            (ndone + nleft);
    ...
    if (end_time_us > top_time_us) {
        ...
        if (n == _time_q.capacity() ||
            ...
            // will not overflow.
            scaled_qps =
                (n - 1) * 1000000L * WEIGHT_SCALE / (end_time_us - top_time_us);
            if (scaled_qps < WEIGHT_SCALE) {
                scaled_qps = WEIGHT_SCALE;
            }
        }
        _avg_latency =
            (_time_q.bottom()->latency_sum - _time_q.top()->latency_sum) /
            (n - 1);
    } else if (n == 1) {
...
void LocalityAwareLoadBalancer::Destroy() { delete this; }

void LocalityAwareLoadBalancer::Weight::Describe(std::ostream& os,
                                                 int64_t now) {
...
void LocalityAwareLoadBalancer::Describe(std::ostream& os,
                                         const DescribeOptions& options) {
    if (!options.verbose) {
        os << "la";
        return;
    }
    os << "LocalityAware{total=" << _total.load(butil::memory_order_relaxed)
       << ' ';
    butil::DoublyBufferedData<Servers>::ScopedPtr s;
    if (_db_servers.Read(&s) != 0) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            os << " left=" << info.left->load(butil::memory_order_relaxed)
               << ' ';
