...
DEFINE_int32(h2_client_header_table_size, H2Settings::DEFAULT_HEADER_TABLE_SIZE,
...
DEFINE_int32(
    h2_client_max_frame_size, H2Settings::DEFAULT_MAX_FRAME_SIZE,
...
static bool CheckStreamWindowSize(const char*, int32_t val) { return val >= 0; }
...
static const char* H2ConnectionState2Str(H2ConnectionState s) {
    switch (s) {
    case H2_CONNECTION_UNINITIALIZED:
        return "UNINITIALIZED";
    case H2_CONNECTION_READY:
        return "READY";
    case H2_CONNECTION_GOAWAY:
        return "GOAWAY";
    }
...
inline void SerializeFrameHead(void* out_buf, const H2FrameHead& h) {
    return SerializeFrameHead(out_buf, h.payload_size, h.type, h.flags,
                              h.stream_id);
}
...
inline bool AddWindowSize(butil::atomic<int64_t>* window_size, int64_t diff) {
    ...
    int64_t before_add =
        window_size->fetch_add(diff, butil::memory_order_relaxed);
...
inline bool MinusWindowSize(butil::atomic<int64_t>* window_size, int64_t size) {
    ...
    int64_t before_sub =
        window_size->fetch_sub(size, butil::memory_order_relaxed);
    if (before_sub < size) {
...
H2Context::H2Context(Socket* socket, const Server* server)
    ...
    if (server) {
        ...
        _unack_local_settings.header_table_size =
            FLAGS_h2_client_header_table_size;
        _unack_local_settings.stream_window_size =
            FLAGS_h2_client_stream_window_size;
        _unack_local_settings.max_frame_size = FLAGS_h2_client_max_frame_size;
        _unack_local_settings.connection_window_size =
            FLAGS_h2_client_connection_window_size;
    }
#if defined(UNIT_TEST)
    // In ut, we hope _last_sent_stream_id run out quickly to test the
    // correctness of creating new h2 socket. This value is 10,000 less than
    // 0x7FFFFFFF.
...
void H2Context::RemoveGoAwayStreams(
    ...
    if (goaway_stream_id == 0) {  // quick path
        ...
        for (StreamMap::const_iterator it = tmp.begin(); it != tmp.end();
             ++it) {
...
ParseResult H2Context::ConsumeFrameHead(butil::IOBufBytesIterator& it,
    ...
    if (length > _local_settings.max_frame_size) {
        LOG(ERROR) << "Too large frame length=" << length
                   << " max=" << _local_settings.max_frame_size;
...
ParseResult H2Context::Consume(butil::IOBufBytesIterator& it, Socket* socket) {
    if (_conn_state == H2_CONNECTION_UNINITIALIZED) {
        if (is_server_side()) {
            ...
            const size_t nb = SerializeH2SettingsFrameAndWU(
                _unack_local_settings, settingsbuf);
            if (WriteAck(socket, settingsbuf, nb) != 0) {
                LOG(WARNING) << "Fail to respond http2-client with settings to "
                             << *socket;
                return MakeParseError(PARSE_ERROR_ABSOLUTELY_WRONG);
            }
        } else {
        ...
        if (h2_res.stream_id()) {  // send RST_STREAM
            ...
            SerializeFrameHead(rstbuf, 4, H2_FRAME_RST_STREAM, 0,
                               h2_res.stream_id());
...
H2ParseResult H2Context::OnHeaders(butil::IOBufBytesIterator& it,
                                   const H2FrameHead& frame_head) {
...
H2ParseResult H2StreamContext::OnHeaders(butil::IOBufBytesIterator& it,
                                         const H2FrameHead& frame_head,
                                         uint32_t frag_size,
                                         uint8_t pad_length) {
    ...
    if (it2.bytes_left()) {
        it.append_and_forward(&_remaining_header_fragment, it2.bytes_left());
    }
    it.forward(pad_length);
    if (frame_head.flags & H2_FLAGS_END_HEADERS) {
        if (it2.bytes_left() != 0) {
            LOG(ERROR) << "Incomplete header: payload_size="
...
H2ParseResult H2Context::OnContinuation(butil::IOBufBytesIterator& it,
                                        const H2FrameHead& frame_head) {
...
H2ParseResult H2StreamContext::OnContinuation(butil::IOBufBytesIterator& it,
                                              const H2FrameHead& frame_head) {
    _parsed_length += FRAME_HEAD_SIZE + frame_head.payload_size;
...
H2ParseResult H2Context::OnData(butil::IOBufBytesIterator& it,
                                const H2FrameHead& frame_head) {
    uint32_t frag_size = frame_head.payload_size;
    uint8_t pad_length = 0;
    ...
    if (sctx == NULL) {
        ...
        // "half-closed (local)" state, the recipient MUST respond with a stream
        // error (Section 5.4.2) of type STREAM_CLOSED. Ignore the message
...
H2ParseResult H2StreamContext::OnData(butil::IOBufBytesIterator& it,
                                      const H2FrameHead& frame_head,
    ...
    const int64_t acc = _deferred_window_update.fetch_add(
                            frag_size, butil::memory_order_relaxed) +
                        frag_size;
    if (acc >= _conn_ctx->local_settings().stream_window_size / 2) {
        ...
        if (stream_wu > 0) {
            ...
            const int64_t conn_wu =
                stream_wu + _conn_ctx->ReleaseDeferredWindowUpdate();
            SerializeFrameHead(p, 4, H2_FRAME_WINDOW_UPDATE, 0, 0);
            SaveUint32(p + FRAME_HEAD_SIZE, conn_wu);
            if (WriteAck(_conn_ctx->_socket, winbuf, sizeof(winbuf)) != 0) {
                LOG(WARNING)
                    << "Fail to send WINDOW_UPDATE to " << *_conn_ctx->_socket;
...
H2ParseResult H2Context::OnResetStream(butil::IOBufBytesIterator& it,
                                       const H2FrameHead& frame_head) {
...
H2ParseResult H2StreamContext::OnResetStream(H2Error h2_error,
                                             const H2FrameHead& frame_head) {
    _parsed_length += FRAME_HEAD_SIZE + frame_head.payload_size;
#if defined(BRPC_H2_STREAM_STATE)
...
H2ParseResult H2Context::OnSettings(butil::IOBufBytesIterator& it,
                                    const H2FrameHead& frame_head) {
    ...
    if (!_remote_settings_received) {
        ...
        // receving remote setting, the initial window size of stream/connection
        // is set to MAX_WINDOW_SIZE(see constructor of H2Context). As a result,
        // in the view of remote side, window size is 65535 by default so it may
        // not send its stream size to sender, making stream size still be
...
H2ParseResult H2Context::OnPriority(butil::IOBufBytesIterator&,
                                    const H2FrameHead&) {
    LOG(ERROR) << "Not support PRIORITY frame yet";
    return MakeH2Error(H2_PROTOCOL_ERROR);
}

H2ParseResult H2Context::OnPushPromise(butil::IOBufBytesIterator&,
                                       const H2FrameHead&) {
    LOG(ERROR) << "Not support PUSH_PROMISE frame yet";
    return MakeH2Error(H2_PROTOCOL_ERROR);
}

H2ParseResult H2Context::OnPing(butil::IOBufBytesIterator& it,
                                const H2FrameHead& frame_head) {
...
H2ParseResult H2Context::OnGoAway(butil::IOBufBytesIterator& it,
                                  const H2FrameHead& h) {
    if (h.payload_size < 8) {
    ...
    if (is_client_side()) {
        ...
        for (size_t i = 1; i < goaway_streams.size(); ++i) {
            ...
            CHECK_EQ(0, bthread_start_background(
                            &th, &tmp, ProcessHttpResponseWrapper,
...
H2ParseResult H2Context::OnWindowUpdate(butil::IOBufBytesIterator& it,
                                        const H2FrameHead& frame_head) {
    if (frame_head.payload_size != 4) {
        LOG(ERROR) << "Invalid payload_size=" << frame_head.payload_size;
        return MakeH2Error(H2_FRAME_SIZE_ERROR);
    ...
    if (frame_head.stream_id == 0) {
        if (!AddWindowSize(&_remote_window_left, inc)) {
            LOG(ERROR) << "Invalid connection-level window_size_increment="
                       << inc;
            return MakeH2Error(H2_FLOW_CONTROL_ERROR);
        }
        return MakeH2Message(NULL);
...
void H2Context::Describe(std::ostream& os, const DescribeOptions& opt) const {
    ...
    os << sep << "last_received_stream_id=" << _last_received_stream_id << sep
       << "last_sent_stream_id=" << _last_sent_stream_id;
    os << sep << "deferred_window_update="
       << _deferred_window_update.load(butil::memory_order_relaxed) << sep
       << "remote_conn_window_left="
       << _remote_window_left.load(butil::memory_order_relaxed) << sep
       << "remote_settings=" << _remote_settings << sep
       << "remote_settings_received=" << _remote_settings_received << sep
       << "local_settings=" << _local_settings << sep << "hpacker={";
    ...
    os << sep << "abandoned_streams=" << abandoned_size << sep
       << "pending_streams=" << VolatilePendingStreamSize();
    if (opt.verbose) {
        os << '\n';
    }
}
...
void H2Context::DeferWindowUpdate(int64_t size) {
    ...
    const int64_t acc =
        _deferred_window_update.fetch_add(size, butil::memory_order_relaxed) +
        size;
...
bool H2StreamContext::ConsumeWindowSize(int64_t size) {
    ...
    int64_t after_sub =
        _remote_window_left.fetch_sub(size, butil::memory_order_relaxed) - size;
    if (after_sub < 0) {
...
static void PackH2Message(butil::IOBuf* out, butil::IOBuf& headers,
                          butil::IOBuf& trailer_headers,
                          const butil::IOBuf& data, int stream_id,
                          H2Context* conn_ctx) {
    const H2Settings& remote_settings = conn_ctx->remote_settings();
    char headbuf[FRAME_HEAD_SIZE];
    H2FrameHead headers_head = {(uint32_t)headers.size(), H2_FRAME_HEADERS, 0,
                                stream_id};
    ...
    if (!trailer_headers.empty()) {
        H2FrameHead headers_head = {(uint32_t)trailer_headers.size(),
                                    H2_FRAME_HEADERS, 0, stream_id};
        headers_head.flags |= H2_FLAGS_END_STREAM;
        headers_head.flags |= H2_FLAGS_END_HEADERS;
        SerializeFrameHead(headbuf, headers_head);
        out->append(headbuf, sizeof(headbuf));
...
H2UnsentRequest* H2UnsentRequest::New(Controller* c) {
    ...
    const size_t maxsize = h.HeaderCount() + 4 + (size_t)need_content_type +
                           (size_t)need_accept + (size_t)need_user_agent +
                           (size_t)need_authorization;
    const size_t memsize =
        offsetof(H2UnsentRequest, _list) + sizeof(HPacker::Header) * maxsize;
    ...
    if (scheme->empty()) {
        scheme =
            (c->is_ssl() ? &common->H2_SCHEME_HTTPS : &common->H2_SCHEME_HTTP);
    }
        ...
        if (!uri.host().empty()) {
            if (uri.port() < 0) {
                *val = uri.host();
                ...
                butil::string_printf(val, "%s:%d", uri.host().c_str(),
                                     uri.port());
            }
...
butil::Status H2UnsentRequest::AppendAndDestroySelf(butil::IOBuf* out,
                                                    Socket* socket) {
#if defined(BRPC_PROFILE_H2)
    ...
    if (!_cntl->request_attachment().empty()) {
        ...
        if (!_sctx->ConsumeWindowSize(data_size)) {
            return butil::Status(
                ELIMIT, "remote_window_left is not enough, data_size=%" PRId64,
                data_size);
        }
    }
    ...
    // After calling TryToInsertStream, the ownership of _sctx is transferred to
    // ctx
    _sctx.release();
    ...
    PackH2Message(out, frag, dummy_buf, _cntl->request_attachment(), _stream_id,
                  ctx);
    return butil::Status::OK();
}
...
H2UnsentResponse* H2UnsentResponse::New(Controller* c, int stream_id,
    ...
    const CommonStrings* const common = get_common_strings();
    const bool need_content_type      = !h->content_type().empty();
    const size_t maxsize              = 1 + (size_t)need_content_type;
    const size_t memsize =
        offsetof(H2UnsentResponse, _list) + sizeof(HPacker::Header) * maxsize;
    H2UnsentResponse* msg =
        new (malloc(memsize)) H2UnsentResponse(c, stream_id, is_grpc);
    // :status
    if (h->status_code() == 200) {
        msg->push(common->H2_STATUS, common->STATUS_200);
    } else {
        butil::string_printf(&msg->push(common->H2_STATUS), "%d",
                             h->status_code());
    }
    if (need_content_type) {
        msg->push(common->CONTENT_TYPE, h->content_type());
...
butil::Status H2UnsentResponse::AppendAndDestroySelf(butil::IOBuf* out,
                                                     Socket* socket) {
#if defined(BRPC_PROFILE_H2)
...
void PackH2Request(butil::IOBuf*, SocketMessage** user_message,
                   uint64_t correlation_id,
                   const google::protobuf::MethodDescriptor*, Controller* cntl,
                   const butil::IOBuf&, const Authenticator* auth) {
...
StreamUserData* H2GlobalStreamCreator::OnCreatingStream(SocketUniquePtr* inout,
                                                        Controller* cntl) {
