...
inline void PackSofaHeader(char* sofa_header, int meta_size, int body_size) {
    uint32_t* dummy = reinterpret_cast<uint32_t*>(
        sofa_header);  // suppress strict-alias warning
...
static void SerializeSofaHeaderAndMeta(butil::IOBuf* out,
                                       const SofaRpcMeta& meta,
                                       int payload_size) {
    ...
    if (meta_size <= 232) {  // most common cases
        ...
        ::google::protobuf::io::ArrayOutputStream arr_out(header_and_meta + 24,
                                                          meta_size);
...
static void SendSofaResponse(int64_t correlation_id, Controller* cntl,
    ...
    meta.set_compress_type(CompressType2Sofa(cntl->response_compress_type()));
...
void ProcessSofaRequest(InputMessageBase* msg_base) {
    ...
    do {
        ...
        google::protobuf::Closure* done =
            ::brpc::NewCallback<int64_t, Controller*,
                ...
                &SendSofaResponse, correlation_id, cntl.get(), req.get(),
    ...
    // `socket' will be held until response has been sent
    SendSofaResponse(correlation_id, cntl.release(), req.release(),
...
void ProcessSofaResponse(InputMessageBase* msg_base) {
    ...
    if (meta.error_code() != 0) {
        ...
        cntl->SetFailed(meta.error_code(), "%s", meta.reason().c_str());
        ...
        if (!ParseFromCompressedData(msg->payload, cntl->response(),
                                     res_cmp_type)) {
            cntl->SetFailed(ERESPONSE,
...
void PackSofaRequest(butil::IOBuf* req_buf, SocketMessage**,
    ...
    if (method) {
        ...
        meta.set_compress_type(
            CompressType2Sofa(cntl->request_compress_type()));
