...
DEFINE_int32(
    rtmp_server_window_ack_size, 2500000,
...
DEFINE_string(user_defined_data_message, "",
              "extra name that user can specify in Data Message of RTMP, "
              "handled by OnMetaData");
...
const char* messagetype2str(RtmpMessageType t) {
    ...
    case RTMP_MESSAGE_SET_CHUNK_SIZE:
        return "SetChunkSize";
    case RTMP_MESSAGE_ABORT:
        return "AbortMessage";
    case RTMP_MESSAGE_ACK:
        return "Ack";
    case RTMP_MESSAGE_USER_CONTROL:
        return "UserControlMessage";
    case RTMP_MESSAGE_WINDOW_ACK_SIZE:
        return "WindowAckSize";
    case RTMP_MESSAGE_SET_PEER_BANDWIDTH:
        return "SetPeerBandwidth";
    case RTMP_MESSAGE_AUDIO:
        return "AudioMessage";
    case RTMP_MESSAGE_VIDEO:
        return "VideoMessage";
    case RTMP_MESSAGE_DATA_AMF3:
        return "DataMessage_AMF3";
    case RTMP_MESSAGE_SHARED_OBJECT_AMF3:
        return "SharedObjectMessage_AMF3";
    case RTMP_MESSAGE_COMMAND_AMF3:
        return "CommandMessage_AMF3";
    case RTMP_MESSAGE_DATA_AMF0:
        return "DataMessage_AMF0";
    case RTMP_MESSAGE_SHARED_OBJECT_AMF0:
        return "SharedObjectMessage_AMF0";
    case RTMP_MESSAGE_COMMAND_AMF0:
        return "CommandMessage_AMF0";
    case RTMP_MESSAGE_AGGREGATE:
...
static uint32_t GetRtmpTimestamp() { return 0; }
...
int openssl_HMACsha256(const void* key, int key_size, const void* data,
    ...
    if (NULL == EVP_sha256) {
        LOG_ONCE(ERROR)
            << "Fail to find EVP_sha256, fall back to simple handshaking";
        return -1;
    }
    ...
    if (key == NULL) {
        ...
        if (HMAC(EVP_sha256(), key, key_size, (const unsigned char*)data,
                 data_size, temp_digest, &digest_size) == NULL) {
...
public:
    ...
    bool Load(const void* key, int key_size, const void* c1s1_digest,
              const void* buf);
    void Save(void* buf) const;

private:
    bool ComputeDigest(const void* key, int key_size, const void* c1s1_digest,
                       void* digest) const;
...
bool C1::Load(const void* buf) {
    ...
    if (memcmp(tmp_digest, digest_blk.digest(), DigestBlock::DIGEST_SIZE) ==
        0) {
        return true;
    }
...
bool S1::Generate(const C1& c1) {
    ...
    int pkey_size = 128;
    if (dh.copy_shared_key(c1.key_blk.key(), 128, key_blk.key(), &pkey_size) !=
        0) {  // took ~0.9ms
...
static void WriteBasicHeader(char** buf, RtmpChunkType chunk_type,
                             uint32_t cs_id) {
    char* out = *buf;
    if (cs_id < 2) {
...
const char* RtmpContext::state2str(State s) {
    ...
    case STATE_RECEIVED_S0S1:
        return "STATE_RECEIVED_S0S1";
    case STATE_RECEIVED_S2:
        return "STATE_RECEIVED_S2";
    case STATE_RECEIVED_C0C1:
        return "STATE_RECEIVED_C0C1";
...
void RtmpContext::SetState(const butil::EndPoint& remote_side,
                           State new_state) {
...
RtmpUnsentMessage* MakeUnsentControlMessage(uint8_t message_type,
...
RtmpUnsentMessage* MakeUnsentControlMessage(uint8_t message_type,
                                            const void* data, size_t n) {
    return MakeUnsentControlMessage(message_type, RTMP_CONTROL_CHUNK_STREAM_ID,
                                    data, n);
}
...
RtmpContext::~RtmpContext() {
    if (!_mstream_map.empty()) {
        size_t ncstream = 0;
        size_t nsstream = 0;
        for (butil::FlatMap<uint32_t, MessageStreamInfo>::iterator it =
                 _mstream_map.begin();
             it != _mstream_map.end(); ++it) {
        ...
        _mstream_map.clear();
        LOG(FATAL) << "RtmpContext=" << this
                   << " is deallocated"
                      " before all streams("
                   << ncstream << " client, " << nsstream
                   << "server) on the connection quit";
    }

    // cancel incomplete transactions
    for (butil::FlatMap<uint32_t, RtmpTransactionHandler*>::iterator it =
             _trans_map.begin();
         it != _trans_map.end(); ++it) {
        if (it->second) {
            it->second->Cancel();
        }
    }
...
void RtmpContext::Destroy() { delete this; }

butil::Status RtmpUnsentMessage::AppendAndDestroySelf(butil::IOBuf* out,
                                                      Socket* s) {
...
void RtmpContext::ClearChunkStream(uint32_t cs_id) {
    ...
    delete sub_array->ptrs[index2].exchange(NULL, butil::memory_order_acquire);
}
...
void RtmpContext::DeallocateChunkStreamId(uint32_t chunk_stream_id) {
    // NOTE: duplicated id may be pushed into _free_cs_ids, not affecting
    // correctness.
...
RtmpTransactionHandler* RtmpContext::RemoveTransaction(
...
int RtmpContext::SendConnectRequest(const butil::EndPoint& remote_side, int fd,
...
ParseResult RtmpContext::WaitForC0C1orSimpleRtmp(butil::IOBuf* source,
                                                 Socket* socket) {
    ...
    if (memcmp(p + RTMP_HANDSHAKE_SIZE0, SIMPLIFIED_RTMP_MAGIC_NUMBER,
               MAGIC_NUMBER_SIZE) == 0) {
    ...
    if (c1.Load(c0c1_buf + RTMP_HANDSHAKE_SIZE0)) {
        ...
        tmp.push_back(RTMP_DEFAULT_VERSION);
        {
            ...
            if (!s1.Generate(c1)) {
                LOG(WARNING)
                    << socket->remote_side() << ": Fail to generate s1";
                return MakeParseError(PARSE_ERROR_NO_RESOURCE);
            }
...
ParseResult RtmpContext::WaitForS0S1(butil::IOBuf* source, Socket* socket) {
    ...
    if (!_only_check_simple_s0s1) {
        ...
        if (s1.Load(s0s1_buf + RTMP_HANDSHAKE_SIZE0, adobe_hs::SCHEMA1)) {
            ...
            RPC_VLOG << socket->remote_side()
                     << ": Fallback to simple handshaking";
        }
    }
    if (!done_adobe_hs) {
        // Send back C2
...
ParseResult RtmpContext::WaitForS2(butil::IOBuf* source, Socket* socket) {
    ...
    if (SendConnectRequest(socket->remote_side(), socket->fd(), false) != 0) {
        LOG(ERROR) << "Fail to send connect request to "
                   << socket->remote_side();
        return MakeParseError(PARSE_ERROR_NO_RESOURCE);
    }
    return OnChunks(source, socket);
...
RtmpChunkStream::RtmpChunkStream(RtmpContext* conn_ctx, uint32_t cs_id)
    : _conn_ctx(conn_ctx), _cs_id(cs_id) {}
...
ParseResult RtmpChunkStream::Feed(const RtmpBasicHeader& bh,
    ...
    case RTMP_CHUNK_TYPE0: {
        ...
        const char* p =
            (const char*)source->fetch(buf, header_len) + bh.header_length;
    ...
    case RTMP_CHUNK_TYPE3: {
        ...
        if (!_r.last_has_extended_ts) {
            ...
            if (!_r.first_chunk_of_message && timestamp_delta > 0 &&
                timestamp_delta != _r.last_timestamp_delta) {
                header_len -= 4;
                timestamp_delta = _r.last_timestamp_delta;
            }
    ...
    VLOG(vlvl) << socket->remote_side()
               ...
               << " fmt=" << bh.fmt << " body_size=" << cur_chunk_size << '}';
...
typedef butil::FlatMap<std::string, RtmpChunkStream::CommandHandler>
    CommandHandlerMap;
...
static void InitCommandHandlers() {
    ...
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_CREATE_STREAM] =
        &RtmpChunkStream::OnCreateStream;
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_DELETE_STREAM] =
        &RtmpChunkStream::OnDeleteStream;
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_CLOSE_STREAM] =
    ...
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_RELEASE_STREAM] =
        &RtmpChunkStream::OnReleaseStream;
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_FC_PUBLISH] =
        &RtmpChunkStream::OnFCPublish;
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_FC_UNPUBLISH] =
        &RtmpChunkStream::OnFCUnpublish;
    (*s_cmd_handlers)[RTMP_AMF0_COMMAND_GET_STREAM_LENGTH] =
        &RtmpChunkStream::OnGetStreamLength;
...
bool RtmpChunkStream::OnMessage(const RtmpBasicHeader& bh,
    ...
    if (index >= arraysize(s_msg_handlers)) {
        RTMP_ERROR(socket, mh)
            << "Unknown message_type=" << (int)mh.message_type;
        return false;
    }
...
bool RtmpChunkStream::OnSetChunkSize(const RtmpMessageHeader& mh,
                                     butil::IOBuf* msg_body, Socket* socket) {
    if (mh.message_length != 4u) {
        RTMP_ERROR(socket, mh)
            << "Expected message_length=4, actually " << mh.message_length;
        return false;
    }
    char buf[4];
    ...
    if (new_size > 0x7FFFFFFF) {
        RTMP_ERROR(socket, mh)
            << "MSB of chunk_size=" << new_size << " is not zero";
        return false;
    }
...
bool RtmpChunkStream::OnAbortMessage(const RtmpMessageHeader& mh,
                                     butil::IOBuf* msg_body, Socket* socket) {
    if (mh.message_length != 4u) {
        RTMP_ERROR(socket, mh)
            << "Expected message_length=4, actually " << mh.message_length;
...
bool RtmpChunkStream::OnAck(const RtmpMessageHeader& mh, butil::IOBuf* msg_body,
                            Socket* socket) {
    if (mh.message_length != 4u) {
        RTMP_ERROR(socket, mh)
            << "Expected message_length=4, actually " << mh.message_length;
...
bool RtmpChunkStream::OnWindowAckSize(const RtmpMessageHeader& mh,
                                      butil::IOBuf* msg_body, Socket* socket) {
    if (mh.message_length != 4u) {
        RTMP_ERROR(socket, mh)
            << "Expected message_length=4, actually " << mh.message_length;
...
bool RtmpChunkStream::OnSetPeerBandwidth(const RtmpMessageHeader& mh,
                                         butil::IOBuf* msg_body,
                                         Socket* socket) {
    if (mh.message_length != 5u) {
        RTMP_ERROR(socket, mh)
            << "Expected message_length=5, actually " << mh.message_length;
        return false;
    }
    char buf[5];
    msg_body->cutn(buf, sizeof(buf));
...
bool RtmpChunkStream::OnUserControlMessage(const RtmpMessageHeader& mh,
                                           butil::IOBuf* msg_body,
                                           Socket* socket) {
...
bool RtmpChunkStream::OnStreamBegin(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid StreamBegin.event_data.size=" << event_data.size();
        return false;
    }
    // Ignore StreamBegin
    return true;
...
bool RtmpChunkStream::OnStreamEOF(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid StreamEOF.event_data.size=" << event_data.size();
        return false;
    }
    // Ignore StreamEOF
    return true;
...
bool RtmpChunkStream::OnStreamDry(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid StreamDry.event_data.size=" << event_data.size();
        return false;
    }
    // Ignore StreamDry
    return true;
...
bool RtmpChunkStream::OnStreamIsRecorded(const RtmpMessageHeader& mh,
    ...
    if (service != NULL) {
        RTMP_ERROR(socket, mh)
            << "Server should not receive `StreamIsRecorded'";
        return false;
    }
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid StreamIsRecorded.event_data.size=" << event_data.size();
        return false;
    }
...
bool RtmpChunkStream::OnSetBufferLength(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 8) {
        RTMP_ERROR(socket, mh)
            << "Invalid SetBufferLength.event_data.size=" << event_data.size();
        return false;
    }
...
bool RtmpChunkStream::OnPingRequest(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid PingRequest.event_data.size=" << event_data.size();
        return false;
    }
    ...
    SocketMessagePtr<RtmpUnsentMessage> msg(MakeUnsentControlMessage(
        RTMP_MESSAGE_USER_CONTROL, data, sizeof(data)));
...
bool RtmpChunkStream::OnPingResponse(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid PingResponse.event_data.size=" << event_data.size();
        return false;
    }
    const uint32_t timestamp = ReadBigEndian4Bytes(event_data.data());
...
bool RtmpChunkStream::OnBufferEmpty(const RtmpMessageHeader& mh,
    ...
    // we may consider the pause/unpause trick used in
    // http://repo.or.cz/w/rtmpdump.git/blob/8880d1456b282ee79979adbe7b6a6eb8ad371081:/librtmp/rtmp.c#l2787
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid BufferEmpty.event_data.size=" << event_data.size();
        return false;
    }
...
bool RtmpChunkStream::OnBufferReady(const RtmpMessageHeader& mh,
    ...
    if (event_data.size() != 4) {
        RTMP_ERROR(socket, mh)
            << "Invalid BufferReady.event_data.size=" << event_data.size();
        return false;
    }
    const uint32_t tmp = ReadBigEndian4Bytes(event_data.data());
    const int vlvl     = RPC_VLOG_LEVEL + 1;
...
bool RtmpChunkStream::OnAudioMessage(const RtmpMessageHeader& mh,
                                     butil::IOBuf* msg_body, Socket* socket) {
    char first_byte = 0;
...
bool RtmpChunkStream::OnVideoMessage(const RtmpMessageHeader& mh,
                                     butil::IOBuf* msg_body, Socket* socket) {
    char first_byte = 0;
    if (!msg_body->cut1(&first_byte)) {
        // pretty common, don't print logs.
    ...
    if (!is_video_frame_type_valid(msg.frame_type)) {
        RTMP_WARNING(socket, mh)
            << "Invalid frame_type=" << (int)msg.frame_type;
    }
...
bool RtmpChunkStream::OnDataMessageAMF0(const RtmpMessageHeader& mh,
                                        butil::IOBuf* msg_body,
                                        Socket* socket) {
    ...
    RPC_VLOG << socket->remote_side() << "[" << mh.stream_id
             << "] DataMessage{timestamp=" << mh.timestamp << " name=" << name
             << '}';

    if (name == RTMP_AMF0_ON_META_DATA ||
        name == FLAGS_user_defined_data_message) {
...
bool RtmpChunkStream::OnSharedObjectMessageAMF0(const RtmpMessageHeader&,
                                                butil::IOBuf*, Socket* socket) {
    LOG_EVERY_SECOND(ERROR) << socket->remote_side() << ": Not implemented";
    return false;
}

bool RtmpChunkStream::OnCommandMessageAMF0(const RtmpMessageHeader& mh,
                                           butil::IOBuf* msg_body,
                                           Socket* socket) {
    butil::IOBufAsZeroCopyInputStream zc_stream(*msg_body);
    AMFInputStream istream(&zc_stream);
...
bool RtmpChunkStream::OnDataMessageAMF3(const RtmpMessageHeader& mh,
                                        butil::IOBuf* msg_body,
                                        Socket* socket) {
...
bool RtmpChunkStream::OnSharedObjectMessageAMF3(const RtmpMessageHeader&,
                                                butil::IOBuf*, Socket*) {
    LOG(ERROR) << "Not implemented";
    return false;
}

bool RtmpChunkStream::OnCommandMessageAMF3(const RtmpMessageHeader& mh,
                                           butil::IOBuf* msg_body,
                                           Socket* socket) {
    msg_body->pop_front(1);
...
bool RtmpChunkStream::OnAggregateMessage(const RtmpMessageHeader&,
                                         butil::IOBuf*, Socket*) {
    LOG(ERROR) << "Not implemented";
    return false;
}
...
bool RtmpChunkStream::OnConnect(const RtmpMessageHeader& mh,
    ...
    RPC_VLOG << socket->remote_side() << "[" << mh.stream_id << "] connect{"
    ...
    msgs.push().reset(MakeUnsentControlMessage(RTMP_MESSAGE_WINDOW_ACK_SIZE,
                                               wasbuf, sizeof(wasbuf)));
    ...
    *p++ = RTMP_LIMIT_DYNAMIC;
    msgs.push().reset(MakeUnsentControlMessage(RTMP_MESSAGE_SET_PEER_BANDWIDTH,
                                               spbbuf, sizeof(spbbuf)));
    ...
    msgs.push().reset(MakeUnsentControlMessage(RTMP_MESSAGE_COMMAND_AMF0,
                                               chunk_stream_id(), req_buf));
    ...
    RPC_VLOG << socket->remote_side() << ": respond connect, props={"
             << response.ShortDebugString() << "} info={"
             << info.ShortDebugString() << '}';
    return true;
}

bool RtmpChunkStream::OnBWDone(const RtmpMessageHeader& mh, AMFInputStream*,
                               Socket* socket) {
    RPC_VLOG << socket->remote_side() << "[" << mh.stream_id
             << "] ignore onBWDone";
    return true;
    ...
    if (transaction_id < 2) {
        if (transaction_id == 1) {
            ...
            if (!_conn_ctx->_simplified_rtmp) {
                // In simplified rtmp case,
                // connection_context()->_create_stream_with_play_or_publish is
                // set and OnConnected is called in RtmpConnect::StartConnect,
                // so we don't need to do these operations again here.
                if (connect_res.create_stream_with_play_or_publish()) {
                    connection_context()->_create_stream_with_play_or_publish =
                        true;
                }
...
bool RtmpChunkStream::OnStatus(const RtmpMessageHeader& mh,
    ...
    RPC_VLOG << socket->remote_side() << "[" << mh.stream_id << "] onStatus{"
             << info.ShortDebugString() << '}';
...
bool RtmpChunkStream::OnPlay(const RtmpMessageHeader& mh,
    ...
    RPC_VLOG << socket->remote_side() << "[" << mh.stream_id
             ...
             << " start=" << play_opt.start << " duration=" << play_opt.duration
    ...
    // StreamBegin
    {
        ...
        msgs.push().reset(MakeUnsentControlMessage(RTMP_MESSAGE_USER_CONTROL,
                                                   cntl_buf, sizeof(cntl_buf)));
    }
    // Play.Reset
    if (play_opt.reset) {
    ...
    RtmpServerStream* stream =
        static_cast<RtmpServerStream*>(stream_guard.get());
...
bool RtmpChunkStream::OnCloseStream(const RtmpMessageHeader& mh,
    ...
    if (!connection_context()->FindMessageStream(mh.stream_id, &stream)) {
        // TODO: frequent, commented now
        // RTMP_WARNING(socket, mh) << "Fail to find stream_id=" <<
        // mh.stream_id;
        return false;
    }
    if (!stream->_paused) {
    ...
    if (!status.ok()) {
        ...
        if (FLAGS_log_error_text) {
            LOG(WARNING) << "Error to " << publish_stream->remote_side() << '['
                         << publish_stream->stream_id() << "]: " << status;
        }
        return;
    }
    butil::IOBuf req_buf;
...
bool RtmpChunkStream::OnReleaseStream(const RtmpMessageHeader& mh,
                                      AMFInputStream* istream, Socket* socket) {
    if (!connection_context()->is_server_side()) {
...
bool RtmpChunkStream::OnFCPublish(const RtmpMessageHeader& mh,
                                  AMFInputStream* istream, Socket* socket) {
    if (!connection_context()->is_server_side()) {
...
bool RtmpChunkStream::OnFCUnpublish(const RtmpMessageHeader& mh,
                                    AMFInputStream* istream, Socket* socket) {
    if (!connection_context()->is_server_side()) {
...
bool RtmpChunkStream::OnGetStreamLength(const RtmpMessageHeader&,
                                        AMFInputStream*, Socket*) {
...
bool RtmpChunkStream::OnCheckBW(const RtmpMessageHeader&, AMFInputStream*,
                                Socket*) {
    return true;
}
...
bool RtmpChunkStream::OnPause(const RtmpMessageHeader& mh,
    ...
    int rc = static_cast<RtmpServerStream*>(stream.get())
                 ->OnPause(pause_or_unpause, milliseconds);
    ...
    if (WriteWithoutOvercrowded(socket, msg1) != 0) {
        PLOG(WARNING) << socket->remote_side() << '[' << mh.stream_id
                      << "] Fail to respond "
                      << (pause_or_unpause ? "pause" : "unpause");
        return false;
    }
    if (rc == 0) {
...
ParseResult ParseRtmpMessage(butil::IOBuf* source, Socket* socket,
                             bool read_eof, const void* arg) {
    RtmpContext* rtmp_ctx =
        static_cast<RtmpContext*>(socket->parsing_context());
    if (rtmp_ctx == NULL) {
        if (arg == NULL) {
...
public:
    ...
    void Run(bool error, const RtmpMessageHeader& mh, AMFInputStream* istream,
             Socket* socket);
    void Cancel();
...
OnServerStreamCreated::OnServerStreamCreated(RtmpClientStream* stream,
                                             CallId call_id)
    : _stream(stream), _call_id(call_id) {}

void OnServerStreamCreated::Run(bool error, const RtmpMessageHeader&,
                                AMFInputStream* istream, Socket* socket) {
    ...
    do {
        ...
        if (!ctx->AddClientStream(_stream.get())) {
            cntl->SetFailed(EINVAL, "Fail to add client stream_id=%u",
                            stream_id);
            break;
        }
    } while (0);
...
void OnServerStreamCreated::Cancel() { delete this; }

butil::Status RtmpCreateStreamMessage::AppendAndDestroySelf(butil::IOBuf* out,
                                                            Socket* s) {
    ...
    RtmpChunkStream* cstream =
        ctx->GetChunkStream(RTMP_CONTROL_CHUNK_STREAM_ID);
    if (cstream == NULL) {
...
void PackRtmpRequest(butil::IOBuf* /*buf*/, SocketMessage** user_message,
    ...
    OnServerStreamCreated* cb =
        new OnServerStreamCreated(stream, cntl->call_id());
...
void SerializeRtmpRequest(butil::IOBuf* /*buf*/, Controller* /*cntl*/,
