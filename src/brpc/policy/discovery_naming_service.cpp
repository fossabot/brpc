...
DEFINE_string(discovery_api_addr, DEFAULT_DISCOVERY_API_ADDR,
...
static int ListDiscoveryNodes(const char* discovery_api_addr,
                              std::string* servers) {
    Channel api_channel;
    ...
    for (BUTIL_RAPIDJSON_NAMESPACE::SizeType i = 0; i < data.Size(); ++i) {
        ...
        if (itr_addr == addr_item.MemberEnd() || !itr_addr->value.IsString() ||
            itr_status == addr_item.MemberEnd() ||
            !itr_status->value.IsUint() || itr_status->value.GetUint() != 0) {
            continue;
        }
        servers->push_back(',');
...
static void NewDiscoveryChannel() {
    ...
    if (ListDiscoveryNodes(FLAGS_discovery_api_addr.c_str(),
                           &discovery_servers) != 0) {
        LOG(ERROR) << "Fail to get discovery nodes from "
    ...
    if (s_discovery_channel->Init(discovery_servers.c_str(), "rr",
                                  &channel_options) != 0) {
...
DiscoveryClient::DiscoveryClient() : _th(INVALID_BTHREAD), _registered(false) {}
...
static int ParseCommonResult(const butil::IOBuf& buf, std::string* error_text) {
    ...
    if (itr_message != d.MemberEnd() && itr_message->value.IsString() &&
        error_text) {
...
int DiscoveryClient::DoRenew() const {
    ...
    os << "appid=" << _params.appid << "&hostname=" << _params.hostname
       << "&env=" << _params.env << "&region=" << _params.region
       << "&zone=" << _params.zone;
    ...
    if (ParseCommonResult(cntl.response_attachment(), &error_text) != 0) {
        LOG(ERROR) << "Fail to renew " << _params.hostname << " to "
                   << _params.appid << ": " << error_text;
        return -1;
    }
    return 0;
}
...
void* DiscoveryClient::PeriodicRenew(void* arg) {
    ...
    int64_t init_sleep_s =
        FLAGS_discovery_renew_interval_s / 2 +
    ...
    while (!bthread_stopped(bthread_self())) {
        if (consecutive_renew_error == FLAGS_discovery_reregister_threshold) {
            LOG(WARNING)
                << "Re-register since discovery renew error threshold reached";
...
int DiscoveryClient::DoRegister() {
    ...
    os << "&env=" << _params.env << "&zone=" << _params.zone
       << "&region=" << _params.region << "&status=" << _params.status
       << "&version=" << _params.version << "&metadata=" << _params.metadata;
    os.move_to(cntl.request_attachment());
    chan->CallMethod(NULL, &cntl, NULL, NULL, NULL);
    if (cntl.Failed()) {
        LOG(ERROR) << "Fail to register " << _params.appid << ": "
                   << cntl.ErrorText();
        return -1;
    }
    std::string error_text;
    if (ParseCommonResult(cntl.response_attachment(), &error_text) != 0) {
        LOG(ERROR) << "Fail to register " << _params.hostname << " to "
                   << _params.appid << ": " << error_text;
        return -1;
    }
...
int DiscoveryClient::DoCancel() const {
    ...
    std::string error_text;
    if (ParseCommonResult(cntl.response_attachment(), &error_text) != 0) {
        LOG(ERROR) << "Fail to cancel " << _params.hostname << " in "
                   << _params.appid << ": " << error_text;
        return -1;
    }
    return 0;
...
int DiscoveryNamingService::GetServers(const char* service_name,
    ...
    if (service_name == NULL || *service_name == '\0' ||
        FLAGS_discovery_env.empty() || FLAGS_discovery_status.empty()) {
    ...
    for (BUTIL_RAPIDJSON_NAMESPACE::SizeType i = 0; i < instances.Size(); ++i) {
        ...
        if (itr_metadata != instances[i].MemberEnd()) {
            ...
            BUTIL_RAPIDJSON_NAMESPACE::Writer<
                BUTIL_RAPIDJSON_NAMESPACE::MemoryBuffer>
                writer(buffer);
        ...
        for (BUTIL_RAPIDJSON_NAMESPACE::SizeType j = 0; j < addrs.Size(); ++j) {
            ...
            butil::StringPiece addr(addrs[j].GetString(),
                                    addrs[j].GetStringLength());
...
void DiscoveryNamingService::Destroy() { delete this; }
