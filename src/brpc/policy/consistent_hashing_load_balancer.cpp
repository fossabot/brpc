...
public:
    ...
    virtual bool Build(
        ServerId server, size_t num_replicas,
...
bool DefaultReplicaPolicy::Build(
    ServerId server, size_t num_replicas,
    std::vector<ConsistentHashingLoadBalancer::Node>* replicas) const {
    SocketUniquePtr ptr;
...
bool KetamaReplicaPolicy::Build(
    ServerId server, size_t num_replicas,
    std::vector<ConsistentHashingLoadBalancer::Node>* replicas) const {
    SocketUniquePtr ptr;
    if (Socket::AddressFailedAsWell(server.id, &ptr) == -1) {
...
void InitReplicaPolicy() {
    g_replica_policy = new std::array<const ReplicaPolicy*, CONS_HASH_LB_LAST>(
        {new DefaultReplicaPolicy(MurmurHash32),
         new DefaultReplicaPolicy(MD5Hash32), new KetamaReplicaPolicy});
}

inline const ReplicaPolicy* GetReplicaPolicy(
...
size_t ConsistentHashingLoadBalancer::AddBatch(std::vector<Node>& bg,
                                               const std::vector<Node>& fg,
                                               const std::vector<Node>& servers,
                                               bool* executed) {
    if (*executed) {
        // Hack DBD
    ...
    *executed = true;
    ...
    bg.resize(std::set_union(fg.begin(), fg.end(), servers.begin(),
                             servers.end(), bg.begin()) -
              bg.begin());
...
size_t ConsistentHashingLoadBalancer::Remove(std::vector<Node>& bg,
                                             const std::vector<Node>& fg,
...
bool ConsistentHashingLoadBalancer::AddServer(const ServerId& server) {
    ...
    bool executed = false;
    const size_t ret =
        _db_hash_ring.ModifyWithForeground(AddBatch, add_nodes, &executed);
...
size_t ConsistentHashingLoadBalancer::AddServersInBatch(
    ...
    for (size_t i = 0; i < servers.size(); ++i) {
        replicas.clear();
        if (GetReplicaPolicy(_type)->Build(servers[i], _num_replicas,
                                           &replicas)) {
...
bool ConsistentHashingLoadBalancer::RemoveServer(const ServerId& server) {
    bool executed = false;
    const size_t ret =
        _db_hash_ring.ModifyWithForeground(Remove, server, &executed);
...
size_t ConsistentHashingLoadBalancer::RemoveServersInBatch(
    ...
    bool executed = false;
    const size_t ret =
        _db_hash_ring.ModifyWithForeground(RemoveBatch, servers, &executed);
    CHECK(ret % _num_replicas == 0);
    const size_t n = ret / _num_replicas;
...
void ConsistentHashingLoadBalancer::Destroy() { delete this; }

int ConsistentHashingLoadBalancer::SelectServer(const SelectIn& in,
                                                SelectOut* out) {
...
void ConsistentHashingLoadBalancer::Describe(std::ostream& os,
    ...
    for (std::map<butil::EndPoint, double>::iterator it = load_map.begin();
         it != load_map.end(); ++it) {
...
void ConsistentHashingLoadBalancer::GetLoads(
    ...
    for (std::map<butil::EndPoint, uint32_t>::iterator it = count_map.begin();
         it != count_map.end(); ++it) {
        (*load_map)[it->first] = (double)it->second / UINT_MAX;
    }
}

bool ConsistentHashingLoadBalancer::SetParameters(
