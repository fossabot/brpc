...
AutoConcurrencyLimiter* AutoConcurrencyLimiter::New(
    ...
    if (last_sampling_time_us == 0 ||
        ...
        if (sample_this_call) {
            ...
            if (sample_window_submitted) {
                ...
                VLOG(1) << "Sample window submitted, current max_concurrency:"
                        << _max_concurrency
...
int AutoConcurrencyLimiter::MaxConcurrency() { return _max_concurrency; }
...
bool AutoConcurrencyLimiter::AddSample(int error_code, int64_t latency_us,
...
void AutoConcurrencyLimiter::UpdateMinLatency(int64_t latency_us) {
    ...
    if (_min_latency_us <= 0) {
        ...
        _min_latency_us =
            latency_us * ema_factor + _min_latency_us * (1 - ema_factor);
    }
}
...
void AutoConcurrencyLimiter::AdjustMaxConcurrency(int next_max_concurrency) {
    next_max_concurrency =
        std::max(bthread::FLAGS_bthread_concurrency, next_max_concurrency);
...
void AutoConcurrencyLimiter::UpdateMaxConcurrency(int64_t sampling_time_us) {
    ...
    double qps =
        1000000.0 * total_succ_req / (sampling_time_us - _sw.start_time_us);
    ...
    if (_remeasure_start_us <= sampling_time_us) {
        ...
        if (avg_latency <= _min_latency_us *
                               (1.0 + min_explore_ratio * correction_factor) ||
            qps <= _ema_max_qps / (1.0 + min_explore_ratio)) {
            _explore_ratio =
                std::min(max_explore_ratio, _explore_ratio + change_step);
        } else {
            _explore_ratio =
                std::max(min_explore_ratio, _explore_ratio - change_step);
        }
        next_max_concurrency =
