...
bool CircuitBreaker::EmaErrorRecorder::OnCallEnd(int error_code,
    ...
    if (_sample_count_when_initializing.load(butil::memory_order_relaxed) <
            _window_size &&
        _sample_count_when_initializing.fetch_add(
            1, butil::memory_order_relaxed) < _window_size) {
        if (error_code != 0) {
            const int32_t error_count =
                _error_count_when_initializing.fetch_add(
                    1, butil::memory_order_relaxed);
            return error_count < _window_size * _max_error_percent / 100;
        }
        // Because once OnCallEnd returned false, the node will be ioslated
        // soon, so when error_code=0, we no longer check the error count.
        return true;
    }
...
bool CircuitBreaker::EmaErrorRecorder::UpdateErrorCost(int64_t error_cost,
    ...
    if (error_cost != 0) {
        ...
        const int64_t max_error_cost = ema_latency * _window_size *
                                       (_max_error_percent / 100.0) *
                                       (1.0 + EPSILON);
    ...
    do {
        if (ema_error_cost == 0) {
            ...
            if (_ema_error_cost.compare_exchange_weak(ema_error_cost,
                                                      next_ema_error_cost)) {
                break;
            }
        }
...
void CircuitBreaker::UpdateIsolationDuration() {
    ...
    int isolation_duration_ms =
        _isolation_duration_ms.load(butil::memory_order_relaxed);
    ...
    _isolation_duration_ms.store(isolation_duration_ms,
