...
public:
    ...
    const static uint64_t MAX_NUM_PER_THREAD =
        (1ul << (64ul - SUM_BIT_WIDTH)) - 1;
    ...
    typedef detail::ReducerSampler<IntRecorder, Stat, AddStat, MinusStat>
        sampler_type;
    ...
    IntRecorder(const butil::StringPiece& prefix,
    ...
    Stat get_value() const { return _combiner.combine_agents(); }

    Stat reset() { return _combiner.reset_all_agents(); }
...
private:
    ...
    static uint64_t _get_num(const uint64_t n) { return n >> SUM_BIT_WIDTH; }

    // Fill all the first (64 - SUM_BIT_WIDTH + 1) bits with 1 if the sign bit
    // is 1 to represent a complete 64-bit negative number Check out
    // http://en.wikipedia.org/wiki/Signed_number_representations if you are
    // confused
